/* Class422 - Decompiled by JODE
 * Visit http://jode.sourceforge.net/
 */
package com.jagex;

public class IncomingPacket {
	public static IncomingPacket aClass422_4591;
	public static IncomingPacket aClass422_4592;
	public static IncomingPacket aClass422_4593;
	public static IncomingPacket aClass422_4594;
	public static IncomingPacket aClass422_4595;
	public static IncomingPacket aClass422_4596;
	public static IncomingPacket aClass422_4597;
	public static IncomingPacket aClass422_4598;
	public static IncomingPacket SET_INTERFACE;
	public static IncomingPacket aClass422_4600;
	public static IncomingPacket aClass422_4601;
	public static IncomingPacket SOUND_EFFECT;
	public static IncomingPacket aClass422_4603;
	public static IncomingPacket aClass422_4604;
	public static IncomingPacket SEND_ITEMS;
	public static IncomingPacket aClass422_4606;
	public static IncomingPacket aClass422_4607;
	public static IncomingPacket aClass422_4608;
	public static IncomingPacket aClass422_4609;
	public static IncomingPacket aClass422_4610;
	public static IncomingPacket aClass422_4611;
	public static IncomingPacket aClass422_4612;
	public static IncomingPacket aClass422_4613;
	public static IncomingPacket aClass422_4614;
	public static IncomingPacket aClass422_4615;
	public static IncomingPacket CS_VAR_STRING_2;
	public static IncomingPacket ADD_GROUND_ITEM;
	public static IncomingPacket aClass422_4618;
	public static IncomingPacket aClass422_4619;
	public static IncomingPacket aClass422_4620;
	public static IncomingPacket aClass422_4621;
	public static IncomingPacket aClass422_4622;
	public static IncomingPacket PING;
	public static IncomingPacket aClass422_4624;
	public static IncomingPacket ICOMPONENT_TEXT;
	public static IncomingPacket aClass422_4626;
	public static IncomingPacket aClass422_4627;
	public static IncomingPacket aClass422_4628;
	public static IncomingPacket aClass422_4629;
	public static IncomingPacket NPC_UPDATE;
	public static IncomingPacket RUN_ENERGY;
	public static IncomingPacket CS_VAR_1;
	public static IncomingPacket aClass422_4633;
	public static IncomingPacket GAME_MESSAGE;
	public static IncomingPacket aClass422_4635;
	public static IncomingPacket aClass422_4636;
	public static IncomingPacket aClass422_4637;
	public static IncomingPacket aClass422_4638;
	public static IncomingPacket aClass422_4639;
	public static IncomingPacket aClass422_4640;
	public static IncomingPacket aClass422_4641;
	public static IncomingPacket aClass422_4642;
	public static IncomingPacket aClass422_4643;
	public static IncomingPacket aClass422_4644;
	public static IncomingPacket PLAYER_UPDATE;
	public static IncomingPacket aClass422_4646;
	public static IncomingPacket aClass422_4647;
	public static IncomingPacket aClass422_4648;
	public static IncomingPacket SKILLS;
	public static IncomingPacket aClass422_4650;
	public static IncomingPacket aClass422_4651;
	public static IncomingPacket aClass422_4652;
	public static IncomingPacket aClass422_4653;
	public static IncomingPacket aClass422_4654;
	public static IncomingPacket aClass422_4655;
	public static IncomingPacket aClass422_4656;
	public static IncomingPacket MAP_REGION;
	public static IncomingPacket aClass422_4658;
	public static IncomingPacket aClass422_4659;
	public static IncomingPacket aClass422_4660;
	public static IncomingPacket aClass422_4661;
	public static IncomingPacket aClass422_4662;
	public static IncomingPacket aClass422_4663;
	public static IncomingPacket ROOT_INTERFACE;
	public static IncomingPacket aClass422_4665;
	public static IncomingPacket aClass422_4666;
	public static IncomingPacket aClass422_4667;
	public static IncomingPacket CHUNK_SCENE;
	public static IncomingPacket aClass422_4669;
	public static IncomingPacket aClass422_4670;
	public static IncomingPacket aClass422_4671;
	public static IncomingPacket aClass422_4672;
	public static IncomingPacket aClass422_4673;
	public static IncomingPacket DYNAMIC_MAP_SCENE;
	public static IncomingPacket PLAYER_OPTION;
	public static IncomingPacket aClass422_4676;
	public static IncomingPacket aClass422_4677;
	public static IncomingPacket CS_VAR_2;
	public static IncomingPacket aClass422_4679;
	public static IncomingPacket HIDE_ICOMPONENT;
	public static IncomingPacket aClass422_4681;
	public static IncomingPacket OBJECT_RELATED_MAYBE;
	public static IncomingPacket ICOMPONENT_ANIMATION;
	public static IncomingPacket aClass422_4684;
	public static IncomingPacket aClass422_4685;
	public static IncomingPacket CS_VAR_STRING_1;
	public static IncomingPacket REMOVE_GROUND_ITEM;
	public static IncomingPacket aClass422_4688;
	public static IncomingPacket aClass422_4689;
	public static IncomingPacket aClass422_4690;
	public static IncomingPacket aClass422_4691;
	public static IncomingPacket aClass422_4692;
	public static IncomingPacket aClass422_4693;
	public static IncomingPacket aClass422_4694;
	public static IncomingPacket NPC_HEAD_ON_ICOMPONENT;
	public static IncomingPacket aClass422_4696;
	public static IncomingPacket aClass422_4697;
	public static IncomingPacket aClass422_4698;
	public static IncomingPacket aClass422_4699;
	public static IncomingPacket ITEM_ON_ICOMPONENT;
	public static IncomingPacket aClass422_4701;
	public static IncomingPacket MINIMAP_FLAG;
	public static IncomingPacket aClass422_4703;
	public static IncomingPacket aClass422_4704;
	public static IncomingPacket aClass422_4705;
	public static IncomingPacket CLIENT_SCRIPT;
	public static IncomingPacket aClass422_4707;
	public static IncomingPacket aClass422_4708;
	public static IncomingPacket aClass422_4709;
	public static IncomingPacket aClass422_4710;
	public static IncomingPacket aClass422_4711;
	public static IncomingPacket aClass422_4712;
	public static IncomingPacket aClass422_4713;
	public static IncomingPacket aClass422_4714;
	public static IncomingPacket aClass422_4715 = new IncomingPacket(0, 3);
	public static IncomingPacket aClass422_4716;
	public static IncomingPacket aClass422_4717;
	public static IncomingPacket aClass422_4718;
	public static IncomingPacket VARP_2;
	public static IncomingPacket aClass422_4720;
	public static IncomingPacket aClass422_4721;
	public static IncomingPacket aClass422_4722;
	public static IncomingPacket aClass422_4723;
	public static IncomingPacket aClass422_4724;
	public static IncomingPacket aClass422_4725;
	public static IncomingPacket aClass422_4726;
	public static IncomingPacket aClass422_4727;
	public static IncomingPacket aClass422_4728;
	public static IncomingPacket aClass422_4729;
	public static IncomingPacket aClass422_4730;
	public static IncomingPacket aClass422_4731;
	public static IncomingPacket aClass422_4732;
	public static IncomingPacket VARP_1;
	public static IncomingPacket INTERFACE_SETTINGS;
	static IncomingPacket aClass422_4735;
	public static IncomingPacket aClass422_4736;
	public static IncomingPacket aClass422_4737;
	public static IncomingPacket RESET_NIS_VARS;
	public static IncomingPacket aClass422_4739;
	public static IncomingPacket WORLD_LIST;
	public static IncomingPacket CLOSE_INTERFACE;
	public static IncomingPacket aClass422_4742;
	public static IncomingPacket aClass422_4743;
	public static IncomingPacket aClass422_4744;
	public static IncomingPacket aClass422_4745;
	public static IncomingPacket aClass422_4746;
	public static IncomingPacket ICOMPONENT_SPRITE;
	public static IncomingPacket aClass422_4748;
	public static IncomingPacket aClass422_4749;
	public int anInt4750;
	public static IncomingPacket aClass422_4751;
	public static IncomingPacket aClass422_4752;
	public static IncomingPacket aClass422_4753;
	public static IncomingPacket aClass422_4754;
	public static IncomingPacket aClass422_4755;
	public static IncomingPacket aClass422_4756;
	public static IncomingPacket aClass422_4757;
	public static IncomingPacket aClass422_4758;
	public static IncomingPacket aClass422_4759;
	public static IncomingPacket aClass422_4760;
	public static IncomingPacket aClass422_4761;
	public static IncomingPacket aClass422_4762;
	public static IncomingPacket aClass422_4763;
	public static IncomingPacket aClass422_4764;
	public static IncomingPacket aClass422_4765;
	public static IncomingPacket HINT_ICON;
	public static IncomingPacket PING_2;
	public static IncomingPacket aClass422_4768;
	public static IncomingPacket aClass422_4769;
	public static IncomingPacket aClass422_4770;
	public static IncomingPacket aClass422_4771;
	public static IncomingPacket aClass422_4772;
	public static IncomingPacket aClass422_4773;
	public static IncomingPacket aClass422_4774;
	public static IncomingPacket CLIENT_EDIT_CHECK;
	public static IncomingPacket aClass422_4776;
	public static IncomingPacket aClass422_4777;
	public static IncomingPacket aClass422_4778;
	public static IncomingPacket aClass422_4779;
	public static IncomingPacket aClass422_4780;
	public static IncomingPacket aClass422_4781;
	public static IncomingPacket aClass422_4782;
	public static IncomingPacket PLAYER_HEAD_ON_ICOMPONENT;
	public int id;
	public int realLength;
	public static IncomingPacket aClass422_4785;

	IncomingPacket(int i, int i_0_) {
		id = i * 2069387581;
		anInt4750 = i_0_ * -1415459355;
		realLength = i_0_;
	}

	static {
		aClass422_4592 = new IncomingPacket(1, 3);
		aClass422_4593 = new IncomingPacket(2, 10);
		ICOMPONENT_SPRITE = new IncomingPacket(3, 8);
		aClass422_4756 = new IncomingPacket(4, 8);
		SKILLS = new IncomingPacket(5, 6);
		aClass422_4639 = new IncomingPacket(6, 6);
		aClass422_4598 = new IncomingPacket(7, 1);
		SET_INTERFACE = new IncomingPacket(8, 23);
		aClass422_4600 = new IncomingPacket(9, 2);
		aClass422_4720 = new IncomingPacket(10, -1);
		SOUND_EFFECT = new IncomingPacket(11, 8);
		aClass422_4772 = new IncomingPacket(12, 5);
		aClass422_4604 = new IncomingPacket(13, 25);
		SEND_ITEMS = new IncomingPacket(14, -2);
		aClass422_4765 = new IncomingPacket(15, 8);
		aClass422_4607 = new IncomingPacket(16, 0);
		aClass422_4608 = new IncomingPacket(17, -1);
		aClass422_4609 = new IncomingPacket(18, 0);
		aClass422_4610 = new IncomingPacket(19, 2);
		aClass422_4732 = new IncomingPacket(20, 11);
		aClass422_4647 = new IncomingPacket(21, 2);
		aClass422_4613 = new IncomingPacket(22, 28);
		aClass422_4614 = new IncomingPacket(23, -2);
		aClass422_4615 = new IncomingPacket(24, 7);
		aClass422_4677 = new IncomingPacket(25, 2);
		ADD_GROUND_ITEM = new IncomingPacket(26, 5);
		aClass422_4618 = new IncomingPacket(27, 6);
		aClass422_4619 = new IncomingPacket(28, -1);
		RUN_ENERGY = new IncomingPacket(29, 1);
		aClass422_4621 = new IncomingPacket(30, -2);
		aClass422_4622 = new IncomingPacket(31, 3);
		PING = new IncomingPacket(32, 0);
		aClass422_4624 = new IncomingPacket(33, 10);
		ICOMPONENT_TEXT = new IncomingPacket(34, -2);
		aClass422_4606 = new IncomingPacket(35, -1);
		aClass422_4627 = new IncomingPacket(36, 32);
		aClass422_4628 = new IncomingPacket(37, 0);
		aClass422_4629 = new IncomingPacket(38, 2);
		NPC_UPDATE = new IncomingPacket(39, -2);
		aClass422_4601 = new IncomingPacket(40, 0);
		CS_VAR_1 = new IncomingPacket(41, 3);
		aClass422_4785 = new IncomingPacket(42, -1);
		GAME_MESSAGE = new IncomingPacket(43, -1);
		aClass422_4635 = new IncomingPacket(44, -1);
		CLIENT_SCRIPT = new IncomingPacket(45, -2);
		aClass422_4637 = new IncomingPacket(46, 1);
		aClass422_4764 = new IncomingPacket(47, 0);
		CLOSE_INTERFACE = new IncomingPacket(48, 4);
		aClass422_4595 = new IncomingPacket(49, -2);
		aClass422_4641 = new IncomingPacket(50, 6);
		aClass422_4656 = new IncomingPacket(51, -2);
		aClass422_4643 = new IncomingPacket(52, -2);
		aClass422_4761 = new IncomingPacket(53, 3);
		PLAYER_UPDATE = new IncomingPacket(54, -2);
		aClass422_4646 = new IncomingPacket(55, -2);
		CS_VAR_STRING_1 = new IncomingPacket(56, -1);
		CS_VAR_STRING_2 = new IncomingPacket(57, -2);
		aClass422_4597 = new IncomingPacket(58, 2);
		aClass422_4596 = new IncomingPacket(59, 1);
		aClass422_4651 = new IncomingPacket(60, 2);
		aClass422_4652 = new IncomingPacket(61, 1);
		aClass422_4779 = new IncomingPacket(62, 6);
		aClass422_4650 = new IncomingPacket(63, -2);
		aClass422_4655 = new IncomingPacket(64, -1);
		aClass422_4755 = new IncomingPacket(65, 1);
		MAP_REGION = new IncomingPacket(66, -2);
		aClass422_4658 = new IncomingPacket(67, -1);
		aClass422_4659 = new IncomingPacket(68, 0);
		aClass422_4660 = new IncomingPacket(69, -1);
		aClass422_4648 = new IncomingPacket(70, 3);
		aClass422_4712 = new IncomingPacket(71, 4);
		aClass422_4663 = new IncomingPacket(72, 4);
		ROOT_INTERFACE = new IncomingPacket(73, 19);
		aClass422_4665 = new IncomingPacket(74, 1);
		aClass422_4666 = new IncomingPacket(75, 2);
		aClass422_4667 = new IncomingPacket(76, 6);
		CHUNK_SCENE = new IncomingPacket(77, -2);
		aClass422_4669 = new IncomingPacket(78, -2);
		aClass422_4670 = new IncomingPacket(79, 4);
		aClass422_4671 = new IncomingPacket(80, 6);
		aClass422_4672 = new IncomingPacket(81, 0);
		aClass422_4752 = new IncomingPacket(82, 12);
		aClass422_4640 = new IncomingPacket(83, -2);
		PLAYER_OPTION = new IncomingPacket(84, -1);
		aClass422_4676 = new IncomingPacket(85, 6);
		aClass422_4692 = new IncomingPacket(86, -1);
		CS_VAR_2 = new IncomingPacket(87, 6);
		aClass422_4679 = new IncomingPacket(88, 19);
		aClass422_4636 = new IncomingPacket(89, -1);
		aClass422_4681 = new IncomingPacket(90, -1);
		OBJECT_RELATED_MAYBE = new IncomingPacket(91, 3);
		ICOMPONENT_ANIMATION = new IncomingPacket(92, 8);
		aClass422_4684 = new IncomingPacket(93, -1);
		aClass422_4685 = new IncomingPacket(94, -2);
		aClass422_4745 = new IncomingPacket(95, 1);
		REMOVE_GROUND_ITEM = new IncomingPacket(96, 3);
		aClass422_4688 = new IncomingPacket(97, -1);
		aClass422_4689 = new IncomingPacket(98, -2);
		aClass422_4690 = new IncomingPacket(99, -2);
		aClass422_4691 = new IncomingPacket(100, 0);
		aClass422_4654 = new IncomingPacket(101, 0);
		aClass422_4759 = new IncomingPacket(102, 3);
		aClass422_4694 = new IncomingPacket(103, 2);
		NPC_HEAD_ON_ICOMPONENT = new IncomingPacket(104, 8);
		aClass422_4696 = new IncomingPacket(105, 1);
		aClass422_4697 = new IncomingPacket(106, 7);
		aClass422_4698 = new IncomingPacket(107, 10);
		aClass422_4699 = new IncomingPacket(108, 6);
		ITEM_ON_ICOMPONENT = new IncomingPacket(109, 10);
		aClass422_4642 = new IncomingPacket(110, 18);
		MINIMAP_FLAG = new IncomingPacket(111, 2);
		aClass422_4703 = new IncomingPacket(112, 21);
		aClass422_4771 = new IncomingPacket(113, -2);
		aClass422_4705 = new IncomingPacket(114, 5);
		aClass422_4718 = new IncomingPacket(115, 6);
		aClass422_4707 = new IncomingPacket(116, 25);
		aClass422_4708 = new IncomingPacket(117, 4);
		aClass422_4709 = new IncomingPacket(118, -1);
		aClass422_4710 = new IncomingPacket(119, -1);
		aClass422_4711 = new IncomingPacket(120, 6);
		aClass422_4735 = new IncomingPacket(121, -2);
		aClass422_4713 = new IncomingPacket(122, 2);
		aClass422_4714 = new IncomingPacket(123, 8);
		INTERFACE_SETTINGS = new IncomingPacket(124, 12);
		aClass422_4716 = new IncomingPacket(125, 8);
		aClass422_4717 = new IncomingPacket(126, -2);
		aClass422_4763 = new IncomingPacket(127, 2);
		VARP_2 = new IncomingPacket(128, 6);
		HINT_ICON = new IncomingPacket(129, 14);
		aClass422_4721 = new IncomingPacket(130, -2);
		aClass422_4722 = new IncomingPacket(131, 21);
		aClass422_4644 = new IncomingPacket(132, 8);
		aClass422_4724 = new IncomingPacket(133, 1);
		HIDE_ICOMPONENT = new IncomingPacket(134, 5);
		aClass422_4726 = new IncomingPacket(135, 10);
		aClass422_4727 = new IncomingPacket(136, -1);
		aClass422_4728 = new IncomingPacket(137, 3);
		aClass422_4704 = new IncomingPacket(138, -2);
		aClass422_4730 = new IncomingPacket(139, 10);
		aClass422_4731 = new IncomingPacket(140, -2);
		aClass422_4638 = new IncomingPacket(141, -1);
		VARP_1 = new IncomingPacket(142, 3);
		aClass422_4773 = new IncomingPacket(143, 4);
		aClass422_4751 = new IncomingPacket(144, -1);
		aClass422_4736 = new IncomingPacket(145, 22);
		aClass422_4737 = new IncomingPacket(146, 0);
		RESET_NIS_VARS = new IncomingPacket(147, 0);
		aClass422_4739 = new IncomingPacket(148, 3);
		WORLD_LIST = new IncomingPacket(149, -2);
		aClass422_4729 = new IncomingPacket(150, 0);
		aClass422_4742 = new IncomingPacket(151, 5);
		aClass422_4743 = new IncomingPacket(152, 4);
		aClass422_4744 = new IncomingPacket(153, 10);
		aClass422_4778 = new IncomingPacket(154, 8);
		aClass422_4746 = new IncomingPacket(155, 1);
		aClass422_4591 = new IncomingPacket(156, 6);
		aClass422_4661 = new IncomingPacket(157, 3);
		aClass422_4749 = new IncomingPacket(158, 10);
		aClass422_4781 = new IncomingPacket(159, -1);
		PLAYER_HEAD_ON_ICOMPONENT = new IncomingPacket(160, 4);
		aClass422_4626 = new IncomingPacket(161, 4);
		aClass422_4753 = new IncomingPacket(162, 1);
		aClass422_4754 = new IncomingPacket(163, 6);
		aClass422_4612 = new IncomingPacket(164, -2);
		aClass422_4594 = new IncomingPacket(165, 1);
		aClass422_4757 = new IncomingPacket(166, -1);
		aClass422_4725 = new IncomingPacket(167, 9);
		aClass422_4653 = new IncomingPacket(168, 5);
		aClass422_4760 = new IncomingPacket(169, -2);
		aClass422_4701 = new IncomingPacket(170, 8);
		aClass422_4762 = new IncomingPacket(171, 8);
		aClass422_4603 = new IncomingPacket(172, 29);
		DYNAMIC_MAP_SCENE = new IncomingPacket(173, -2);
		aClass422_4758 = new IncomingPacket(174, -1);
		aClass422_4748 = new IncomingPacket(175, 9);
		PING_2 = new IncomingPacket(176, 0);
		aClass422_4768 = new IncomingPacket(177, 6);
		aClass422_4611 = new IncomingPacket(178, 10);
		aClass422_4770 = new IncomingPacket(179, -2);
		aClass422_4693 = new IncomingPacket(180, -2);
		aClass422_4620 = new IncomingPacket(181, 0);
		aClass422_4673 = new IncomingPacket(182, 0);
		aClass422_4774 = new IncomingPacket(183, 4);
		CLIENT_EDIT_CHECK = new IncomingPacket(184, -2);
		aClass422_4776 = new IncomingPacket(185, 5);
		aClass422_4777 = new IncomingPacket(186, -2);
		aClass422_4633 = new IncomingPacket(187, 2);
		aClass422_4723 = new IncomingPacket(188, 2);
		aClass422_4780 = new IncomingPacket(189, 3);
		aClass422_4769 = new IncomingPacket(190, -2);
		aClass422_4782 = new IncomingPacket(191, 7);
		aClass422_4662 = new IncomingPacket(192, 9);
	}

	static void method5067(Class668 class668, int i) {
		class668.anIntArray8541[1867269829 * class668.anInt8542 - 2] = (((Class277) Class38.aClass53_Sub2_481.method70((class668.anIntArray8541[((1867269829 * class668.anInt8542) - 2)]), (byte) -15)).method3749(Class465_Sub1.aClass96_10352, class668.anIntArray8541[class668.anInt8542 * 1867269829 - 1], 1096469077)) ? 1 : 0;
		class668.anInt8542 -= -1411037171;
	}

	static void method5068(Class668 class668, byte i) {
		class668.anIntArray8541[(class668.anInt8542 += -1411037171) * 1867269829 - 1] = Class300.method4023(-810338655);
	}

	public static int method5069(long l) {
		Class164.method1972(l);
		return Class81.aCalendar859.get(1);
	}

	public static void method5070(byte i) {
		Class710.aClass536_Sub40_8843.method9857((Class710.aClass536_Sub40_8843.aClass710_Sub5_10760), 1, (byte) 1);
		Class710.aClass536_Sub40_8843.method9857((Class710.aClass536_Sub40_8843.aClass710_Sub5_10761), 1, (byte) 1);
		Class710.aClass536_Sub40_8843.method9857((Class710.aClass536_Sub40_8843.aClass710_Sub7_10753), 1, (byte) 1);
		Class710.aClass536_Sub40_8843.method9857((Class710.aClass536_Sub40_8843.aClass710_Sub29_10752), 1, (byte) 1);
		Class710.aClass536_Sub40_8843.method9857((Class710.aClass536_Sub40_8843.aClass710_Sub21_10741), 0, (byte) 1);
		Class710.aClass536_Sub40_8843.method9857((Class710.aClass536_Sub40_8843.aClass710_Sub17_10763), 0, (byte) 1);
		Class710.aClass536_Sub40_8843.method9857((Class710.aClass536_Sub40_8843.aClass710_Sub10_10781), 0, (byte) 1);
		Class710.aClass536_Sub40_8843.method9857((Class710.aClass536_Sub40_8843.aClass710_Sub9_10756), 0, (byte) 1);
		Class710.aClass536_Sub40_8843.method9857((Class710.aClass536_Sub40_8843.aClass710_Sub2_10765), 0, (byte) 1);
		Class710.aClass536_Sub40_8843.method9857((Class710.aClass536_Sub40_8843.aClass710_Sub14_10755), 0, (byte) 1);
		Class710.aClass536_Sub40_8843.method9857((Class710.aClass536_Sub40_8843.aClass710_Sub3_10754), 0, (byte) 1);
		Class710.aClass536_Sub40_8843.method9857((Class710.aClass536_Sub40_8843.aClass710_Sub16_10751), 0, (byte) 1);
		Class710.aClass536_Sub40_8843.method9857((Class710.aClass536_Sub40_8843.aClass710_Sub27_10746), 0, (byte) 1);
		Class710.aClass536_Sub40_8843.method9857((Class710.aClass536_Sub40_8843.aClass710_Sub27_10743), 0, (byte) 1);
		Class710.aClass536_Sub40_8843.method9857((Class710.aClass536_Sub40_8843.aClass710_Sub4_10759), 0, (byte) 1);
		Class710.aClass536_Sub40_8843.method9857(Class710.aClass536_Sub40_8843.aClass710_Sub8_10770, 2043381743 * Class287.aClass287_3208.anInt3207, (byte) 1);
		Class710.aClass536_Sub40_8843.method9857((Class710.aClass536_Sub40_8843.aClass710_Sub34_10787), 0, (byte) 1);
		if (Class677.aClass167_8609 != null && Class677.aClass167_8609.method2013() && Class677.aClass167_8609.method2124())
			Class677.aClass167_8609.method2123();
		Class710.aClass536_Sub40_8843.method9857((Class710.aClass536_Sub40_8843.aClass710_Sub25_10735), 0, (byte) 1);
		Class710.aClass536_Sub40_8843.method9857((Class710.aClass536_Sub40_8843.aClass710_Sub23_10774), 0, (byte) 1);
		Class652.method7869(915220698);
		Class710.aClass536_Sub40_8843.method9857((Class710.aClass536_Sub40_8843.aClass710_Sub33_10762), 2, (byte) 1);
		Class710.aClass536_Sub40_8843.method9857((Class710.aClass536_Sub40_8843.aClass710_Sub43_10780), 2, (byte) 1);
		Class184.method2769(2127077988);
		client.aClass515_11066.method6252(1796531619).method7581((byte) 8);
		client.aBool11026 = true;
	}

	static final void method5071(Class668 class668, int i) {
		Class683 class683 = (class668.aBool8548 ? class668.aClass683_8562 : class668.aClass683_8549);
		Class251 class251 = class683.aClass251_8650;
		Class234 class234 = class683.aClass234_8649;
		Class644.method7791(class251, class234, class668, -1701348869);
	}

	public static void method5072(Class570 class570, int i, int i_1_, Interface59 interface59, byte i_2_) {
		Class668 class668 = Class586.method7063((byte) 5);
		class668.anInterface59_8555 = interface59;
		Class57.method970(class570, i, i_1_, class668, -1818561340);
		class668.anInterface59_8555 = null;
	}

	static final int method5073(Class649_Sub1_Sub5_Sub1 class649_sub1_sub5_sub1, int i) {
		if (0 == class649_sub1_sub5_sub1.anInt11938 * 758944217)
			return 0;
		if (-1 != class649_sub1_sub5_sub1.anInt11911 * -1375730507) {
			Class649_Sub1_Sub5_Sub1 class649_sub1_sub5_sub1_3_ = null;
			if (class649_sub1_sub5_sub1.anInt11911 * -1375730507 < 32768) {
				Class536_Sub13 class536_sub13 = ((Class536_Sub13) (client.aClass4_11050.method556((long) (-1375730507 * class649_sub1_sub5_sub1.anInt11911))));
				if (class536_sub13 != null)
					class649_sub1_sub5_sub1_3_ = ((Class649_Sub1_Sub5_Sub1) class536_sub13.anObject10468);
			} else if (class649_sub1_sub5_sub1.anInt11911 * -1375730507 >= 32768)
				class649_sub1_sub5_sub1_3_ = (client.aClass649_Sub1_Sub5_Sub1_Sub2Array11155[(class649_sub1_sub5_sub1.anInt11911 * -1375730507 - 32768)]);
			if (null != class649_sub1_sub5_sub1_3_) {
				Class436 class436 = Class436.method5252((class649_sub1_sub5_sub1.method7837().aClass436_4823), class649_sub1_sub5_sub1_3_.method7837().aClass436_4823);
				int i_4_ = (int) class436.aFloat4850;
				int i_5_ = (int) class436.aFloat4853;
				if (0 != i_4_ || i_5_ != 0)
					class649_sub1_sub5_sub1.method10867((int) (Math.atan2((double) i_4_, (double) i_5_) * 2607.5945876176133) & 0x3fff, -1733067473);
			}
		}
		if (class649_sub1_sub5_sub1 instanceof Class649_Sub1_Sub5_Sub1_Sub2) {
			Class649_Sub1_Sub5_Sub1_Sub2 class649_sub1_sub5_sub1_sub2 = (Class649_Sub1_Sub5_Sub1_Sub2) class649_sub1_sub5_sub1;
			if (-1 != 1763663333 * class649_sub1_sub5_sub1_sub2.anInt12190 && (-711375609 * class649_sub1_sub5_sub1_sub2.anInt11957 == 0 || (class649_sub1_sub5_sub1_sub2.anInt11915 * 1239423105 > 0))) {
				class649_sub1_sub5_sub1_sub2.method10867(class649_sub1_sub5_sub1_sub2.anInt12190 * 1763663333, -1463114064);
				class649_sub1_sub5_sub1_sub2.anInt12190 = -2114230253;
			}
		} else if (class649_sub1_sub5_sub1 instanceof Class649_Sub1_Sub5_Sub1_Sub1) {
			Class649_Sub1_Sub5_Sub1_Sub1 class649_sub1_sub5_sub1_sub1 = (Class649_Sub1_Sub5_Sub1_Sub1) class649_sub1_sub5_sub1;
			if (2067424907 * class649_sub1_sub5_sub1_sub1.anInt12170 != -1 && (-711375609 * class649_sub1_sub5_sub1_sub1.anInt11957 == 0 || (class649_sub1_sub5_sub1_sub1.anInt11915 * 1239423105 > 0))) {
				Class436 class436 = class649_sub1_sub5_sub1_sub1.method7837().aClass436_4823;
				Class598 class598 = client.aClass515_11066.method6255(-1504285937);
				int i_6_ = ((int) class436.aFloat4850 - (979798784 * class649_sub1_sub5_sub1_sub1.anInt12170 - -1080696064 * class598.anInt7839 - -1080696064 * class598.anInt7839));
				int i_7_ = ((int) class436.aFloat4853 - (class649_sub1_sub5_sub1_sub1.anInt12168 * -1570518272 - 695561472 * class598.anInt7840 - 695561472 * class598.anInt7840));
				if (0 != i_6_ || i_7_ != 0)
					class649_sub1_sub5_sub1_sub1.method10867((int) (Math.atan2((double) i_6_, (double) i_7_) * 2607.5945876176133) & 0x3fff, -1878264216);
				class649_sub1_sub5_sub1_sub1.anInt12170 = 906604253;
			}
		}
		return class649_sub1_sub5_sub1.method10866(-790462894);
	}
}
