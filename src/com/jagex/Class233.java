/* Class233 - Decompiled by JODE
 * Visit http://jode.sourceforge.net/
 */
package com.jagex;

public class Class233 implements Interface75 {
	public static Class233 aClass233_2369;
	public static Class233 aClass233_2370;
	public static Class233 aClass233_2371 = new Class233(2, 0);
	public static Class233 aClass233_2372;
	int anInt2373;
	public static Class233 aClass233_2374;
	public int anInt2375;
	public static Class233 aClass233_2376 = new Class233(0, 1);

	public int method73() {
		return 1801899327 * anInt2373;
	}

	public int method78() {
		return 1801899327 * anInt2373;
	}

	static {
		aClass233_2370 = new Class233(1, 2);
		aClass233_2372 = new Class233(4, 3);
		aClass233_2369 = new Class233(5, 4);
		aClass233_2374 = new Class233(3, 5);
	}

	public int method12() {
		return 1801899327 * anInt2373;
	}

	Class233(int i, int i_0_) {
		anInt2375 = -1550915523 * i;
		anInt2373 = i_0_ * 903193279;
	}

	static void method3328(Class443 class443, byte i) {
		Class70.aClass443_778.method5339(class443);
	}

	static void method3329(Class668 class668, int i) {
		class668.anIntArray8541[1867269829 * class668.anInt8542 - 1] = (((Class277) Class38.aClass53_Sub2_481.method70((class668.anIntArray8541[(class668.anInt8542 * 1867269829) - 1]), (byte) -52)).method3743(Class465_Sub1.aClass96_10352, (byte) 107) ? 1 : 0);
	}

	static final void method3330(IComponentDefinitions class251, Class234 class234, Class668 class668, int i) {
		class251.aBool2608 = (class668.anIntArray8541[(class668.anInt8542 -= -1411037171) * 1867269829]) == 1;
		Class668.method8011(class251, 263642117);
		if (-1 == class251.anInt2546 * -809980533 && !class234.aBool2379)
			Class502.method6041(class251.anInt2559 * -1591767037, (byte) 16);
	}

	public static boolean method3331(int i) {
		Class536_Sub34 class536_sub34 = (Class536_Sub34) Class71.aClass708_794.method8308(1867269829);
		if (class536_sub34 == null)
			return false;
		return true;
	}

	static final void method3332(Class668 class668, byte i) {
		int i_1_ = (class668.anIntArray8541[(class668.anInt8542 -= -1411037171) * 1867269829]);
		IComponentDefinitions class251 = Class264.getIComponentDefinitions(i_1_);
		Class234 class234 = Class463.aClass234Array5227[i_1_ >> 16];
		Class395.method4881(class251, class234, class668, -339463631);
	}

	static final void method3333(Class668 class668, int i) {
		class668.anInt8542 -= 1472892954;
		int i_2_ = class668.anIntArray8541[1867269829 * class668.anInt8542];
		boolean bool = 1 == (class668.anIntArray8541[class668.anInt8542 * 1867269829 + 1]);
		Class672.method8025(i_2_, bool, (byte) -107);
	}

	public static boolean method3334(int i, byte i_3_) {
		return 0 == i || 14 == i || i == 8 || i == 6;
	}

	static final void method3335(Class668 class668, int i) {
		class668.anInt8542 -= 1472892954;
	}

	static void method3336(Class167 class167, byte i) {
		if (Class70.aBool752)
			Class48.method915(class167, (byte) -119);
		else
			Class617.method7367(class167, -1410881436);
	}
}
