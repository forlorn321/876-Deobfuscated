/* Class649_Sub1_Sub4 - Decompiled by JODE
 * Visit http://jode.sourceforge.net/
 */
package com.jagex;

public abstract class Class649_Sub1_Sub4 extends Class649_Sub1 {
	Class536_Sub19[] aClass536_Sub19Array11820;
	protected short aShort11821;
	int anInt11822 = 0;
	protected short aShort11823;

	final void method9976() {
		throw new IllegalStateException();
	}

	boolean method9983() {
		Class436 class436 = method7837().aClass436_4823;
		return (aClass553_10838.aBoolArrayArray7471[(((int) class436.aFloat4850 >> aClass553_10838.anInt7444 * 1893698405) - -1629030593 * aClass553_10838.anInt7454 + aClass553_10838.anInt7447 * 813474573)][(813474573 * aClass553_10838.anInt7447 + (((int) class436.aFloat4853 >> aClass553_10838.anInt7444 * 1893698405) - aClass553_10838.anInt7455 * -1430999951))]);
	}

	boolean method9948(Class167 class167, int i) {
		Class436 class436 = method7837().aClass436_4823;
		return (aClass553_10838.aClass544_7426.method6595(aByte10840, ((int) class436.aFloat4850 >> aClass553_10838.anInt7444 * 1893698405), ((int) class436.aFloat4853 >> 1893698405 * aClass553_10838.anInt7444), method9960(-705575457)));
	}

	final void method9986(int i) {
		throw new IllegalStateException();
	}

	final boolean method9940(int i) {
		return false;
	}

	Class649_Sub1_Sub4(Class553 class553, int i, int i_0_, int i_1_, int i_2_, int i_3_, int i_4_, int i_5_) {
		super(class553);
		aClass536_Sub19Array11820 = new Class536_Sub19[4];
		aByte10839 = (byte) i_2_;
		aByte10840 = (byte) i_3_;
		aShort11823 = (short) i_4_;
		aShort11821 = (short) i_5_;
		method7841(new Class436((float) i, (float) i_0_, (float) i_1_));
		for (int i_6_ = 0; i_6_ < 4; i_6_++)
			aClass536_Sub19Array11820[i_6_] = null;
	}

	boolean method9982(Class167 class167) {
		Class436 class436 = method7837().aClass436_4823;
		return (aClass553_10838.aClass544_7426.method6595(aByte10840, ((int) class436.aFloat4850 >> aClass553_10838.anInt7444 * 1893698405), ((int) class436.aFloat4853 >> 1893698405 * aClass553_10838.anInt7444), method9960(-1765583219)));
	}

	final boolean method9938() {
		return false;
	}

	final boolean method9972() {
		return false;
	}

	final void method9943(Class167 class167, Class649_Sub1 class649_sub1, int i, int i_7_, int i_8_, boolean bool, int i_9_) {
		throw new IllegalStateException();
	}

	final void method9947(Class167 class167, Class649_Sub1 class649_sub1, int i, int i_10_, int i_11_, boolean bool) {
		throw new IllegalStateException();
	}

	final boolean method9973() {
		return false;
	}

	boolean method9949(int i) {
		Class436 class436 = method7837().aClass436_4823;
		return (aClass553_10838.aBoolArrayArray7471[(((int) class436.aFloat4850 >> aClass553_10838.anInt7444 * 1893698405) - -1629030593 * aClass553_10838.anInt7454 + aClass553_10838.anInt7447 * 813474573)][(813474573 * aClass553_10838.anInt7447 + (((int) class436.aFloat4853 >> aClass553_10838.anInt7444 * 1893698405) - aClass553_10838.anInt7455 * -1430999951))]);
	}

	int method9979(Class536_Sub19[] class536_sub19s) {
		if (aBool8468) {
			Class436 class436 = method7837().aClass436_4823;
			anInt11822 = (method9950(((int) class436.aFloat4850 >> aClass553_10838.anInt7444 * 1893698405), ((int) class436.aFloat4853 >> aClass553_10838.anInt7444 * 1893698405), aClass536_Sub19Array11820, 1558158323) * -881980099);
			aBool8468 = false;
		}
		for (int i = 0; i < anInt11822 * -2095107563; i++)
			class536_sub19s[i] = aClass536_Sub19Array11820[i];
		return -2095107563 * anInt11822;
	}

	int method9980(Class536_Sub19[] class536_sub19s) {
		if (aBool8468) {
			Class436 class436 = method7837().aClass436_4823;
			anInt11822 = (method9950(((int) class436.aFloat4850 >> aClass553_10838.anInt7444 * 1893698405), ((int) class436.aFloat4853 >> aClass553_10838.anInt7444 * 1893698405), aClass536_Sub19Array11820, -395447371) * -881980099);
			aBool8468 = false;
		}
		for (int i = 0; i < anInt11822 * -2095107563; i++)
			class536_sub19s[i] = aClass536_Sub19Array11820[i];
		return -2095107563 * anInt11822;
	}

	int method9981(Class536_Sub19[] class536_sub19s) {
		if (aBool8468) {
			Class436 class436 = method7837().aClass436_4823;
			anInt11822 = (method9950(((int) class436.aFloat4850 >> aClass553_10838.anInt7444 * 1893698405), ((int) class436.aFloat4853 >> aClass553_10838.anInt7444 * 1893698405), aClass536_Sub19Array11820, 1744150907) * -881980099);
			aBool8468 = false;
		}
		for (int i = 0; i < anInt11822 * -2095107563; i++)
			class536_sub19s[i] = aClass536_Sub19Array11820[i];
		return -2095107563 * anInt11822;
	}

	int method9959(Class536_Sub19[] class536_sub19s, byte i) {
		if (aBool8468) {
			Class436 class436 = method7837().aClass436_4823;
			anInt11822 = (method9950(((int) class436.aFloat4850 >> aClass553_10838.anInt7444 * 1893698405), ((int) class436.aFloat4853 >> aClass553_10838.anInt7444 * 1893698405), aClass536_Sub19Array11820, -1336129067) * -881980099);
			aBool8468 = false;
		}
		for (int i_12_ = 0; i_12_ < anInt11822 * -2095107563; i_12_++)
			class536_sub19s[i_12_] = aClass536_Sub19Array11820[i_12_];
		return -2095107563 * anInt11822;
	}

	final void method9978(Class167 class167, Class649_Sub1 class649_sub1, int i, int i_13_, int i_14_, boolean bool) {
		throw new IllegalStateException();
	}
}
