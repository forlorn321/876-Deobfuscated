/* Class139 - Decompiled by JODE
 * Visit http://jode.sourceforge.net/
 */
package com.jagex;

public class Class139 implements Interface21 {
	int anInt1648;
	int anInt1649;
	float[] aFloatArray1650;

	public int method82() {
		return 236941179 * anInt1649;
	}

	public int method1() {
		return 236941179 * anInt1649;
	}

	public int method83() {
		return 1079185893 * anInt1648;
	}

	public void method131() {
		/* empty */
	}

	public int method12() {
		return 236941179 * anInt1649;
	}

	public void method129() {
		/* empty */
	}

	public int method78() {
		return 1079185893 * anInt1648;
	}

	public int method73() {
		return 236941179 * anInt1649;
	}

	public int method8() {
		return 1079185893 * anInt1648;
	}

	public int method9() {
		return 1079185893 * anInt1648;
	}

	public void method130() {
		/* empty */
	}

	Class139(int i, int i_0_) {
		anInt1649 = -290252877 * i;
		anInt1648 = i_0_ * -819215379;
		aFloatArray1650 = new float[236941179 * anInt1649 * (anInt1648 * 1079185893)];
	}

	static final void method1696(Class668 class668, int i) {
		int i_1_ = (class668.anIntArray8541[(class668.anInt8542 -= -1411037171) * 1867269829]);
		InterfaceDefinitions class251 = Class264.method3678(i_1_, -689214737);
		Class234 class234 = Class463.aClass234Array5227[i_1_ >> 16];
		Class94.method1292(class251, class234, class668, 1780693201);
	}

	static final void method1697(Class668 class668, byte i) {
		class668.anIntArray8541[(class668.anInt8542 += -1411037171) * 1867269829 - 1] = 1063476305 * Class61.anInt687;
	}

	static final void method1698(Class668 class668, int i) {
		long l = Class249.method3450(1427284439);
		class668.anIntArray8541[(class668.anInt8542 += -1411037171) * 1867269829 - 1] = (int) (l / 86400000L) - 11745;
	}

	public static void method1699(int i, int i_2_) {
		Class536_Sub18_Sub5 class536_sub18_sub5 = Class34_Sub11.method10334(6, (long) i);
		class536_sub18_sub5.method10669(2131277236);
	}

	static final void method1700(Class668 class668, int i) {
		int i_3_ = (class668.anIntArray8541[(class668.anInt8542 -= -1411037171) * 1867269829]);
		InterfaceDefinitions class251 = Class264.method3678(i_3_, -689214737);
		Class234 class234 = Class463.aClass234Array5227[i_3_ >> 16];
		Class15.method642(class251, class234, class668, -279844101);
	}
}
