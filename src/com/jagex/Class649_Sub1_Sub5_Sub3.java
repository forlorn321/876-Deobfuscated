/* Class649_Sub1_Sub5_Sub3 - Decompiled by JODE
 * Visit http://jode.sourceforge.net/
 */
package com.jagex;

public class Class649_Sub1_Sub5_Sub3 extends Class649_Sub1_Sub5 implements Interface59 {
	boolean aBool12006;
	public Class522 aClass522_12007;
	Class559 aClass559_12008;
	boolean aBool12009 = true;
	boolean aBool12010;

	Class563 method9944(GraphicalRenderer class167, byte i) {
		MeshRasterizer class177 = aClass522_12007.method6364(class167, 2048, false, true, -911757926);
		if (null == class177)
			return null;
		Class433 class433 = method7851();
		Class563 class563 = Class225.method3212(aBool12006, 669788563);
		aClass522_12007.method6379(class167, class177, class433, aShort11867, aShort11864, aShort11865, aShort11870, true, (byte) -61);
		ObjTypeDecoder class612 = aClass522_12007.method6365(1206123356);
		if (class612.aClass437_8010 != null) {
			class177.method2490(class433, null, 0);
			class167.method2102(class433, aClass178Array10842[0], class612.aClass437_8010);
		} else
			class177.method2490(class433, aClass178Array10842[0], 0);
		if (aClass522_12007.aClass627_7075 != null) {
			Class166 class166 = aClass522_12007.aClass627_7075.method7476();
			class167.method2351(class166);
		}
		aBool12009 = class177.method2511() || aClass522_12007.aClass627_7075 != null;
		Class431 class431 = method7837();
		if (null == aClass559_12008)
			aClass559_12008 = Class684.method8109((int) class431.aClass436_4823.aFloat4850, (int) class431.aClass436_4823.aFloat4852, (int) class431.aClass436_4823.aFloat4853, class177, (short) -16405);
		else
			Class626.method7463(aClass559_12008, (int) class431.aClass436_4823.aFloat4850, (int) class431.aClass436_4823.aFloat4852, (int) class431.aClass436_4823.aFloat4853, class177, 912355345);
		return class563;
	}

	public Class559 method9963(GraphicalRenderer class167) {
		return aClass559_12008;
	}

	boolean method9942(int i) {
		return aBool12009;
	}

	public boolean method370(int i) {
		return aBool12010;
	}

	public Class559 method9946(GraphicalRenderer class167, int i) {
		return aClass559_12008;
	}

	public int method9953(int i) {
		return aClass522_12007.method6370(11195115);
	}

	boolean method9939(GraphicalRenderer class167, int i, int i_0_, byte i_1_) {
		ObjTypeDecoder class612 = aClass522_12007.method6365(1784273410);
		if (null != class612.aClass437_8010)
			return class167.method2073(i, i_0_, method7851(), class612.aClass437_8010, -1766869920);
		MeshRasterizer class177 = aClass522_12007.method6364(class167, 131072, false, false, -1419095618);
		if (null == class177)
			return false;
		return class177.method2549(i, i_0_, method7851(), false, 0);
	}

	void method9945(GraphicalRenderer class167, int i) {
		MeshRasterizer class177 = aClass522_12007.method6364(class167, 262144, true, true, -1423842717);
		if (class177 != null)
			aClass522_12007.method6379(class167, class177, method7851(), aShort11867, aShort11864, aShort11865, aShort11870, false, (byte) -64);
	}

	public void method10949(ObjTypeTransformation class590, byte i) {
		aClass522_12007.method6363(class590, (byte) -71);
	}

	final void method9978(GraphicalRenderer class167, Class649_Sub1 class649_sub1, int i, int i_2_, int i_3_, boolean bool) {
		throw new IllegalStateException();
	}

	final boolean method9940(int i) {
		return false;
	}

	final void method9943(GraphicalRenderer class167, Class649_Sub1 class649_sub1, int i, int i_4_, int i_5_, boolean bool, int i_6_) {
		throw new IllegalStateException();
	}

	final void method9986(int i) {
		throw new IllegalStateException();
	}

	public int method373(byte i) {
		return -1858685201 * aClass522_12007.anInt7061;
	}

	public int method71(int i) {
		return 1875706389 * aClass522_12007.anInt7069;
	}

	public void method378(int i) {
		/* empty */
	}

	boolean method9969(GraphicalRenderer class167, int i, int i_7_) {
		ObjTypeDecoder class612 = aClass522_12007.method6365(-1250279271);
		if (null != class612.aClass437_8010)
			return class167.method2073(i, i_7_, method7851(), class612.aClass437_8010, -1788876289);
		MeshRasterizer class177 = aClass522_12007.method6364(class167, 131072, false, false, -1997925075);
		if (null == class177)
			return false;
		return class177.method2549(i, i_7_, method7851(), false, 0);
	}

	public boolean method369(int i) {
		return aClass522_12007.method6361(1555630723);
	}

	public int method9958() {
		return aClass522_12007.method6362((byte) 63);
	}

	public void method372(GraphicalRenderer class167, int i) {
		aClass522_12007.method6369(class167, 2141830475);
	}

	public int method9() {
		return -1858685201 * aClass522_12007.anInt7061;
	}

	public int method10() {
		return -1858685201 * aClass522_12007.anInt7061;
	}

	public int method374() {
		return 1875706389 * aClass522_12007.anInt7069;
	}

	public int method267() {
		return 1875706389 * aClass522_12007.anInt7069;
	}

	public int method375() {
		return 1875706389 * aClass522_12007.anInt7069;
	}

	public int method376() {
		return 1515283871 * aClass522_12007.anInt7062;
	}

	public int method377() {
		return 1515283871 * aClass522_12007.anInt7062;
	}

	Class563 method9966(GraphicalRenderer class167) {
		MeshRasterizer class177 = aClass522_12007.method6364(class167, 2048, false, true, -1506062705);
		if (null == class177)
			return null;
		Class433 class433 = method7851();
		Class563 class563 = Class225.method3212(aBool12006, 1961727515);
		aClass522_12007.method6379(class167, class177, class433, aShort11867, aShort11864, aShort11865, aShort11870, true, (byte) -92);
		ObjTypeDecoder class612 = aClass522_12007.method6365(-821317969);
		if (class612.aClass437_8010 != null) {
			class177.method2490(class433, null, 0);
			class167.method2102(class433, aClass178Array10842[0], class612.aClass437_8010);
		} else
			class177.method2490(class433, aClass178Array10842[0], 0);
		if (aClass522_12007.aClass627_7075 != null) {
			Class166 class166 = aClass522_12007.aClass627_7075.method7476();
			class167.method2351(class166);
		}
		aBool12009 = class177.method2511() || aClass522_12007.aClass627_7075 != null;
		Class431 class431 = method7837();
		if (null == aClass559_12008)
			aClass559_12008 = Class684.method8109((int) class431.aClass436_4823.aFloat4850, (int) class431.aClass436_4823.aFloat4852, (int) class431.aClass436_4823.aFloat4853, class177, (short) -17480);
		else
			Class626.method7463(aClass559_12008, (int) class431.aClass436_4823.aFloat4850, (int) class431.aClass436_4823.aFloat4852, (int) class431.aClass436_4823.aFloat4853, class177, -1834702547);
		return class563;
	}

	public void method346() {
		/* empty */
	}

	public void method379() {
		/* empty */
	}

	public void method353() {
		/* empty */
	}

	public Class649_Sub1_Sub5_Sub3(Class553 class553, GraphicalRenderer class167, Class34_Sub17 class34_sub17, ObjTypeDecoder class612, int i, int i_8_, int i_9_, int i_10_, int i_11_, boolean bool, int i_12_, int i_13_, int i_14_, int i_15_, int i_16_, int i_17_, int i_18_, int i_19_, boolean bool_20_) {
		super(class553, i, i_8_, i_9_, i_10_, i_11_, i_12_, i_13_, i_14_, i_15_, 1 == -1529252395 * class612.anInt7996, Class232.method3326(i_16_, i_17_, (byte) -58));
		aClass522_12007 = new Class522(class167, class34_sub17, class612, i_16_, i_17_, i_8_, this, bool, i_18_, i_19_);
		aBool12006 = 1248110961 * class612.anInt7953 != 0 && !bool;
		aBool12010 = bool_20_;
		method9968(1, -771470596);
	}

	public boolean method381() {
		return aClass522_12007.method6361(1373524096);
	}

	public boolean method368() {
		return aBool12010;
	}

	boolean method9974(int i) {
		return false;
	}

	public void method383(GraphicalRenderer class167) {
		aClass522_12007.method6368(class167, 16711935);
	}

	public void method384(GraphicalRenderer class167) {
		aClass522_12007.method6369(class167, 164076759);
	}

	final boolean method9972() {
		return false;
	}

	boolean method9956() {
		return aBool12009;
	}

	public int method9941() {
		return aClass522_12007.method6362((byte) 41);
	}

	public int method367(byte i) {
		return 1515283871 * aClass522_12007.anInt7062;
	}

	boolean method9957(GraphicalRenderer class167, int i, int i_21_) {
		ObjTypeDecoder class612 = aClass522_12007.method6365(-581915445);
		if (null != class612.aClass437_8010)
			return class167.method2073(i, i_21_, method7851(), class612.aClass437_8010, -2013082106);
		MeshRasterizer class177 = aClass522_12007.method6364(class167, 131072, false, false, -1487706255);
		if (null == class177)
			return false;
		return class177.method2549(i, i_21_, method7851(), false, 0);
	}

	public Class559 method9955(GraphicalRenderer class167) {
		return aClass559_12008;
	}

	public Class559 method9961(GraphicalRenderer class167) {
		return aClass559_12008;
	}

	public Class559 method9962(GraphicalRenderer class167) {
		return aClass559_12008;
	}

	public int method9960(int i) {
		return aClass522_12007.method6362((byte) 18);
	}

	public int method9964() {
		return aClass522_12007.method6370(1580419772);
	}

	Class563 method9985(GraphicalRenderer class167) {
		MeshRasterizer class177 = aClass522_12007.method6364(class167, 2048, false, true, -1292702969);
		if (null == class177)
			return null;
		Class433 class433 = method7851();
		Class563 class563 = Class225.method3212(aBool12006, -2123185721);
		aClass522_12007.method6379(class167, class177, class433, aShort11867, aShort11864, aShort11865, aShort11870, true, (byte) -56);
		ObjTypeDecoder class612 = aClass522_12007.method6365(1735517012);
		if (class612.aClass437_8010 != null) {
			class177.method2490(class433, null, 0);
			class167.method2102(class433, aClass178Array10842[0], class612.aClass437_8010);
		} else
			class177.method2490(class433, aClass178Array10842[0], 0);
		if (aClass522_12007.aClass627_7075 != null) {
			Class166 class166 = aClass522_12007.aClass627_7075.method7476();
			class167.method2351(class166);
		}
		aBool12009 = class177.method2511() || aClass522_12007.aClass627_7075 != null;
		Class431 class431 = method7837();
		if (null == aClass559_12008)
			aClass559_12008 = Class684.method8109((int) class431.aClass436_4823.aFloat4850, (int) class431.aClass436_4823.aFloat4852, (int) class431.aClass436_4823.aFloat4853, class177, (short) 1257);
		else
			Class626.method7463(aClass559_12008, (int) class431.aClass436_4823.aFloat4850, (int) class431.aClass436_4823.aFloat4852, (int) class431.aClass436_4823.aFloat4853, class177, -1882661718);
		return class563;
	}

	Class563 method9965(GraphicalRenderer class167) {
		MeshRasterizer class177 = aClass522_12007.method6364(class167, 2048, false, true, -1914143212);
		if (null == class177)
			return null;
		Class433 class433 = method7851();
		Class563 class563 = Class225.method3212(aBool12006, 114071362);
		aClass522_12007.method6379(class167, class177, class433, aShort11867, aShort11864, aShort11865, aShort11870, true, (byte) -74);
		ObjTypeDecoder class612 = aClass522_12007.method6365(1242602922);
		if (class612.aClass437_8010 != null) {
			class177.method2490(class433, null, 0);
			class167.method2102(class433, aClass178Array10842[0], class612.aClass437_8010);
		} else
			class177.method2490(class433, aClass178Array10842[0], 0);
		if (aClass522_12007.aClass627_7075 != null) {
			Class166 class166 = aClass522_12007.aClass627_7075.method7476();
			class167.method2351(class166);
		}
		aBool12009 = class177.method2511() || aClass522_12007.aClass627_7075 != null;
		Class431 class431 = method7837();
		if (null == aClass559_12008)
			aClass559_12008 = Class684.method8109((int) class431.aClass436_4823.aFloat4850, (int) class431.aClass436_4823.aFloat4852, (int) class431.aClass436_4823.aFloat4853, class177, (short) -5762);
		else
			Class626.method7463(aClass559_12008, (int) class431.aClass436_4823.aFloat4850, (int) class431.aClass436_4823.aFloat4852, (int) class431.aClass436_4823.aFloat4853, class177, -318397852);
		return class563;
	}

	public void method371(GraphicalRenderer class167, int i) {
		aClass522_12007.method6368(class167, 16711935);
	}

	void method9967(GraphicalRenderer class167) {
		MeshRasterizer class177 = aClass522_12007.method6364(class167, 262144, true, true, -908926232);
		if (class177 != null)
			aClass522_12007.method6379(class167, class177, method7851(), aShort11867, aShort11864, aShort11865, aShort11870, false, (byte) -28);
	}

	void method9975(GraphicalRenderer class167) {
		MeshRasterizer class177 = aClass522_12007.method6364(class167, 262144, true, true, -1302115336);
		if (class177 != null)
			aClass522_12007.method6379(class167, class177, method7851(), aShort11867, aShort11864, aShort11865, aShort11870, false, (byte) -111);
	}

	final boolean method9938() {
		return false;
	}

	public void method382(GraphicalRenderer class167) {
		aClass522_12007.method6368(class167, 16711935);
	}

	public boolean method380() {
		return aClass522_12007.method6361(1629079601);
	}

	boolean method9971(GraphicalRenderer class167, int i, int i_22_) {
		ObjTypeDecoder class612 = aClass522_12007.method6365(1482445108);
		if (null != class612.aClass437_8010)
			return class167.method2073(i, i_22_, method7851(), class612.aClass437_8010, -1955936697);
		MeshRasterizer class177 = aClass522_12007.method6364(class167, 131072, false, false, -1335978815);
		if (null == class177)
			return false;
		return class177.method2549(i, i_22_, method7851(), false, 0);
	}

	boolean method9987() {
		return false;
	}

	public void method270() {
		/* empty */
	}

	final boolean method9973() {
		return false;
	}

	final void method9947(GraphicalRenderer class167, Class649_Sub1 class649_sub1, int i, int i_23_, int i_24_, boolean bool) {
		throw new IllegalStateException();
	}

	public int method9984() {
		return aClass522_12007.method6362((byte) 17);
	}

	final void method9976() {
		throw new IllegalStateException();
	}
}
