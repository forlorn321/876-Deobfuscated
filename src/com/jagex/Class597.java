/* Class597 - Decompiled by JODE
 * Visit http://jode.sourceforge.net/
 */
package com.jagex;

public class Class597 implements Interface75 {
	static Class597 aClass597_7831;
	static Class597 aClass597_7832;
	static Class597 aClass597_7833;
	static Class597 aClass597_7834;
	static Class597 aClass597_7835 = new Class597(0);
	int anInt7836;
	static Class597 aClass597_7837;

	Class597(int i) {
		anInt7836 = i * 859746275;
	}

	public int method78() {
		return anInt7836 * 1861488075;
	}

	static {
		aClass597_7832 = new Class597(1);
		aClass597_7833 = new Class597(2);
		aClass597_7834 = new Class597(3);
		aClass597_7831 = new Class597(4);
		aClass597_7837 = new Class597(5);
	}

	public int method12() {
		return anInt7836 * 1861488075;
	}

	public int method73() {
		return anInt7836 * 1861488075;
	}

	public static int method7139(Class678 class678, Class665 class665, int i, int i_0_) {
		if (Class678.aClass678_8619 == class678)
			return 80;
		if (Class665.aClass665_8530 == class665)
			return i + 12000;
		return 7000 + i;
	}

	public static int method7140(int i, int i_1_, boolean bool, int i_2_) {
		Class536_Sub1 class536_sub1 = Class659.method7970(i, bool, -1256690210);
		if (class536_sub1 == null)
			return -1;
		if (i_1_ < 0 || i_1_ >= class536_sub1.anIntArray10304.length)
			return -1;
		return class536_sub1.anIntArray10304[i_1_];
	}

	static final void method7141(Class668 class668, int i) {
		class668.anInt8542 -= 1472892954;
		int i_3_ = class668.anIntArray8541[1867269829 * class668.anInt8542];
		int i_4_ = class668.anIntArray8541[1867269829 * class668.anInt8542 + 1];
		Class536_Sub18_Sub12 class536_sub18_sub12 = Class172.method2433(i_3_, i_4_, 74200922);
		if (null != class536_sub18_sub12) {
			class668.anIntArray8541[(class668.anInt8542 += -1411037171) * 1867269829 - 1] = class536_sub18_sub12.anInt11728 * 698041493;
			class668.anObjectArray8543[(class668.anInt8544 += 1946079257) * 366709801 - 1] = Class615.method7333((-8065906675708934543L * class536_sub18_sub12.aLong11731), (Class459.aClass664_5178.anInt8526 * -1919173613), -45986553);
			class668.anIntArray8541[(class668.anInt8542 += -1411037171) * 1867269829 - 1] = class536_sub18_sub12.anInt11733 * 1331173969;
			class668.anObjectArray8543[(class668.anInt8544 += 1946079257) * 366709801 - 1] = (class536_sub18_sub12.aString11727 != null ? class536_sub18_sub12.aString11727 : "");
			class668.anObjectArray8543[(class668.anInt8544 += 1946079257) * 366709801 - 1] = (class536_sub18_sub12.aString11732 != null ? class536_sub18_sub12.aString11732 : "");
			class668.anObjectArray8543[(class668.anInt8544 += 1946079257) * 366709801 - 1] = (class536_sub18_sub12.aString11736 != null ? class536_sub18_sub12.aString11736 : "");
			class668.anObjectArray8543[(class668.anInt8544 += 1946079257) * 366709801 - 1] = (null != class536_sub18_sub12.aString11734 ? class536_sub18_sub12.aString11734 : "");
			class668.anIntArray8541[(class668.anInt8542 += -1411037171) * 1867269829 - 1] = -1293336373 * class536_sub18_sub12.anInt11735;
			class668.anObjectArray8543[(class668.anInt8544 += 1946079257) * 366709801 - 1] = (class536_sub18_sub12.aString11730 != null ? class536_sub18_sub12.aString11730 : "");
			class668.anIntArray8541[(class668.anInt8542 += -1411037171) * 1867269829 - 1] = (class536_sub18_sub12.aClass402_11737 != null ? class536_sub18_sub12.aClass402_11737.method78() : -1);
		} else {
			class668.anIntArray8541[(class668.anInt8542 += -1411037171) * 1867269829 - 1] = -1;
			class668.anObjectArray8543[(class668.anInt8544 += 1946079257) * 366709801 - 1] = "";
			class668.anIntArray8541[(class668.anInt8542 += -1411037171) * 1867269829 - 1] = 0;
			class668.anObjectArray8543[(class668.anInt8544 += 1946079257) * 366709801 - 1] = "";
			class668.anObjectArray8543[(class668.anInt8544 += 1946079257) * 366709801 - 1] = "";
			class668.anObjectArray8543[(class668.anInt8544 += 1946079257) * 366709801 - 1] = "";
			class668.anObjectArray8543[(class668.anInt8544 += 1946079257) * 366709801 - 1] = "";
			class668.anIntArray8541[(class668.anInt8542 += -1411037171) * 1867269829 - 1] = 0;
			class668.anObjectArray8543[(class668.anInt8544 += 1946079257) * 366709801 - 1] = "";
			class668.anIntArray8541[(class668.anInt8542 += -1411037171) * 1867269829 - 1] = -1;
		}
	}
}
