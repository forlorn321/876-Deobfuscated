/* Class210 - Decompiled by JODE
 * Visit http://jode.sourceforge.net/
 */
package com.jagex;

public class Class210 implements Interface75 {
	int anInt2255;
	int anInt2256;
	public static Class210 aClass210_2257;
	public static Class210 aClass210_2258 = new Class210(0, 0);
	public static Class210 aClass210_2259 = new Class210(1, 1);
	static String aString2260;
	public static Class34_Sub18 aClass34_Sub18_2261;

	static {
		aClass210_2257 = new Class210(2, 2);
	}

	public static Class210[] method2980(int i) {
		return (new Class210[] { aClass210_2258, aClass210_2259, aClass210_2257 });
	}

	public int method78() {
		return -1175107927 * anInt2255;
	}

	Class210(int i, int i_0_) {
		anInt2256 = 2130748907 * i;
		anInt2255 = i_0_ * -636568167;
	}

	public int method12() {
		return -1175107927 * anInt2255;
	}

	public int method73() {
		return -1175107927 * anInt2255;
	}

	static final void method2981(Class668 class668, short i) {
		Class683 class683 = (class668.aBool8548 ? class668.aClass683_8562 : class668.aClass683_8549);
		Class251 class251 = class683.aClass251_8650;
		class668.anIntArray8541[(class668.anInt8542 += -1411037171) * 1867269829 - 1] = 223822141 * class251.anInt2574;
	}

	public static Class709 method2982(int i) {
		if (null == Class45.aClass709_521)
			return Class709.aClass709_8828;
		return Class45.aClass709_521;
	}

	static final void method2983(Class668 class668, int i) {
		Class683 class683 = (class668.aBool8548 ? class668.aClass683_8562 : class668.aClass683_8549);
		Class251 class251 = class683.aClass251_8650;
		Class234 class234 = class683.aClass234_8649;
		Class662.method7985(class251, class234, class668, -1335677811);
	}
}
