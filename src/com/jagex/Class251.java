/* Class251 - Decompiled by JODE
 * Visit http://jode.sourceforge.net/
 */
package com.jagex;

public class Class251 {
	public static final int anInt2532 = 2;
	public static int anInt2533;
	public int anInt2534;
	public static int anInt2535;
	public static int anInt2536;
	public static int anInt2537;
	public Object[] anObjectArray2538;
	public static int anInt2539;
	public static int anInt2540;
	public static int anInt2541;
	public static int anInt2542;
	public Object[] anObjectArray2543;
	public Object[] anObjectArray2544;
	public static int anInt2545;
	public int anInt2546;
	public int anInt2547;
	public static int anInt2548;
	public static int anInt2549;
	public static final int anInt2550 = 5;
	public int anInt2551;
	public static Class199 aClass199_2552;
	public Class251 aClass251_2553;
	static Class199 aClass199_2554;
	public Object[] anObjectArray2555;
	public String aString2556;
	public String aString2557;
	public int anInt2558;
	public int anInt2559 = -1002674859;
	public int[] anIntArray2560;
	public int[] anIntArray2561;
	public int anInt2562;
	public byte aByte2563;
	public byte aByte2564;
	public byte aByte2565;
	public String aString2566;
	public int anInt2567;
	static Class199 aClass199_2568;
	public int anInt2569;
	public static int anInt2570;
	public int anInt2571;
	public int anInt2572;
	public int anInt2573;
	public int anInt2574;
	public Class536_Sub14 aClass536_Sub14_2575;
	public int anInt2576;
	public int anInt2577;
	public boolean aBool2578;
	public int anInt2579;
	public int anInt2580;
	public int anInt2581;
	public boolean aBool2582;
	public Class304 aClass304_2583;
	public int anInt2584;
	public int anInt2585;
	public int anInt2586;
	public int anInt2587;
	public int anInt2588;
	public int anInt2589;
	public int anInt2590;
	public static final int anInt2591 = 5;
	public int anInt2592;
	public int anInt2593;
	public boolean aBool2594;
	public int anInt2595;
	public int anInt2596;
	public boolean aBool2597;
	public boolean aBool2598;
	public Object[] anObjectArray2599;
	public int anInt2600;
	public int anInt2601;
	public int anInt2602;
	public int anInt2603;
	public int anInt2604;
	public static int anInt2605;
	public int anInt2606;
	public boolean aBool2607;
	public boolean aBool2608;
	public int anInt2609;
	public int anInt2610;
	public int anInt2611;
	public int anInt2612;
	public int anInt2613;
	public int anInt2614;
	public int anInt2615;
	public boolean aBool2616;
	public static int anInt2617 = 0;
	public byte aByte2618;
	short[] aShortArray2619;
	short[] aShortArray2620;
	short[] aShortArray2621;
	short[] aShortArray2622;
	public int anInt2623;
	public Object[] anObjectArray2624;
	public int anInt2625;
	public int anInt2626;
	public int anInt2627;
	public Object[] anObjectArray2628;
	public int anInt2629;
	Class4 aClass4_2630;
	public int anInt2631;
	public int anInt2632;
	public boolean aBool2633;
	public static int anInt2634;
	public int anInt2635;
	public int anInt2636;
	public boolean aBool2637;
	public String aString2638;
	public int anInt2639;
	public int anInt2640;
	public int anInt2641;
	public boolean aBool2642;
	public int anInt2643;
	public boolean aBool2644;
	public Object[] anObjectArray2645;
	public static int anInt2646;
	public byte[][] aByteArrayArray2647;
	public byte[][] aByteArrayArray2648;
	public boolean aBool2649;
	public int anInt2650;
	public int[] anIntArray2651;
	public static final int anInt2652 = 1;
	public Class688 aClass688_2653;
	public static final int anInt2654 = 4;
	public int anInt2655;
	public static final int anInt2656 = 8;
	public String[] aStringArray2657;
	public int anInt2658;
	public boolean aBool2659;
	public int anInt2660;
	public Object[] anObjectArray2661;
	public Object[] anObjectArray2662;
	public int anInt2663;
	public int anInt2664;
	public String aString2665;
	public boolean aBool2666;
	public Object[] anObjectArray2667;
	public int[] anIntArray2668;
	public Object[] anObjectArray2669;
	public Object[] anObjectArray2670;
	public boolean aBool2671;
	public Object[] anObjectArray2672;
	public Object[] anObjectArray2673;
	public Object[] anObjectArray2674;
	public Object[] anObjectArray2675;
	public Object[] anObjectArray2676;
	public boolean aBool2677;
	public Object[] anObjectArray2678;
	public Object[] anObjectArray2679;
	public int[] anIntArray2680;
	public int anInt2681;
	public boolean aBool2682;
	public Object[] anObjectArray2683;
	public int[] anIntArray2684;
	public static int anInt2685;
	public int[] anIntArray2686;
	public int anInt2687;
	public int[] anIntArray2688;
	public Object[] anObjectArray2689;
	public int[] anIntArray2690;
	public static int anInt2691;
	public boolean aBool2692;
	public int anInt2693;
	public static Class199 aClass199_2694;
	public Object[] anObjectArray2695;
	public Object[] anObjectArray2696;
	public Object[] anObjectArray2697;
	public Object[] anObjectArray2698;
	public String aString2699;
	int anInt2700;
	public Object[] anObjectArray2701;
	public Object[] anObjectArray2702;
	public Object[] anObjectArray2703;
	public Object[] anObjectArray2704;
	public Object[] anObjectArray2705;
	public Object[] anObjectArray2706;
	public Object[] anObjectArray2707;
	public boolean aBool2708;
	public Object[] anObjectArray2709;
	public Object[] anObjectArray2710;
	public int anInt2711;
	public int anInt2712;
	public int anInt2713;
	public Class233 aClass233_2714;
	public Object[] anObjectArray2715;
	public int anInt2716;
	public int anInt2717;
	public int anInt2718;
	int anInt2719;
	public int anInt2720;
	public int anInt2721;
	public int[] anIntArray2722;
	public int anInt2723;
	public static boolean aBool2724;
	public Class251[] aClass251Array2725;
	public Class251[] aClass251Array2726;
	public boolean aBool2727;
	public int anInt2728;
	public int anInt2729;
	public int anInt2730;
	public int anInt2731;
	public int anInt2732;
	public int anInt2733;
	public int anInt2734;
	public int anInt2735;
	public int anInt2736;
	int anInt2737;
	public int[] anIntArray2738;
	public Class627 aClass627_2739;
	public Object[] anObjectArray2740;

	public void method3465(int i, short i_0_, short i_1_, int i_2_) {
		if (i < 5) {
			if (aShortArray2619 == null) {
				aShortArray2619 = new short[5];
				aShortArray2620 = new short[5];
			}
			aShortArray2619[i] = i_0_;
			aShortArray2620[i] = i_1_;
		}
	}

	void method3466(RSByteBuffer class536_sub33, byte i) {
		int i_3_ = class536_sub33.readUnsignedByte(1554604577);
		if (i_3_ == 255)
			i_3_ = -1;
		anInt2728 = class536_sub33.readUnsignedByte(1062980785) * -611060941;
		if ((anInt2728 * 1049444347 & 0x80) != 0) {
			anInt2728 = (1049444347 * anInt2728 & 0x7f) * -611060941;
			aString2557 = class536_sub33.readString((byte) 31);
		}
		anInt2562 = class536_sub33.readUnsignedShort((short) -19136) * -657760679;
		anInt2567 = class536_sub33.readShort((byte) -10) * -45108559;
		anInt2655 = class536_sub33.readShort((byte) -84) * -1160038075;
		anInt2569 = class536_sub33.readUnsignedShort((short) -9217) * 1219788737;
		anInt2664 = class536_sub33.readUnsignedShort((short) -14385) * 49026457;
		aByte2565 = class536_sub33.readByte(-1286798396);
		aByte2618 = class536_sub33.readByte(442057864);
		aByte2563 = class536_sub33.readByte(1193665474);
		aByte2564 = class536_sub33.readByte(-325686991);
		anInt2577 = class536_sub33.readUnsignedShort((short) -24905) * 1248497819;
		if (65535 == -1940204141 * anInt2577)
			anInt2577 = -1248497819;
		else
			anInt2577 = 1248497819 * ((-1591767037 * anInt2559 & ~0xffff) + -1940204141 * anInt2577);
		int i_4_ = class536_sub33.readUnsignedByte(-1371112240);
		aBool2578 = (i_4_ & 0x1) != 0;
		if (i_3_ >= 0)
			aBool2708 = (i_4_ & 0x2) != 0;
		if (1049444347 * anInt2728 == 0) {
			anInt2585 = class536_sub33.readUnsignedShort((short) 10229) * 1043826313;
			anInt2586 = class536_sub33.readUnsignedShort((short) -3456) * -114650895;
			if (i_3_ < 0)
				aBool2708 = class536_sub33.readUnsignedByte(63934850) == 1;
		}
		if (5 == 1049444347 * anInt2728) {
			anInt2592 = class536_sub33.readInt(-1653276395) * -2093350021;
			anInt2593 = class536_sub33.readUnsignedShort((short) -10525) * -207488001;
			int i_5_ = class536_sub33.readUnsignedByte(196273605);
			aBool2594 = (i_5_ & 0x1) != 0;
			aBool2649 = (i_5_ & 0x2) != 0;
			anInt2589 = class536_sub33.readUnsignedByte(-1002164589) * 1266192547;
			anInt2720 = class536_sub33.readUnsignedByte(-394370715) * 483071525;
			anInt2596 = class536_sub33.readInt(-935013261) * -87772689;
			aBool2597 = class536_sub33.readUnsignedByte(824399224) == 1;
			aBool2598 = class536_sub33.readUnsignedByte(-1804809307) == 1;
			anInt2587 = class536_sub33.readInt(-2104015492) * 1647150685;
			if (i_3_ >= 3)
				aBool2677 = class536_sub33.readUnsignedByte(-236424355) == 1;
		}
		if (1049444347 * anInt2728 == 6) {
			anInt2601 = -1782707337;
			anInt2602 = class536_sub33.method9720((byte) 1) * 1639447419;
			int i_6_ = class536_sub33.readUnsignedByte(-1393811510);
			boolean bool = (i_6_ & 0x1) == 1;
			aBool2633 = (i_6_ & 0x2) == 2;
			aBool2616 = 4 == (i_6_ & 0x4);
			aBool2682 = (i_6_ & 0x8) == 8;
			if (bool) {
				anInt2610 = class536_sub33.readShort((byte) -94) * -2137233423;
				anInt2611 = class536_sub33.readShort((byte) -123) * -610004565;
				anInt2551 = class536_sub33.readUnsignedShort((short) 581) * -1501528611;
				anInt2547 = class536_sub33.readUnsignedShort((short) 19413) * -947955085;
				anInt2609 = class536_sub33.readUnsignedShort((short) -734) * 465590379;
				anInt2613 = class536_sub33.readUnsignedShort((short) -1741) * -1926815261;
			} else if (aBool2633) {
				anInt2610 = class536_sub33.readShort((byte) -101) * -2137233423;
				anInt2611 = class536_sub33.readShort((byte) -46) * -610004565;
				anInt2612 = class536_sub33.readShort((byte) -91) * -1069708807;
				anInt2551 = class536_sub33.readUnsignedShort((short) -28922) * -1501528611;
				anInt2547 = class536_sub33.readUnsignedShort((short) 23716) * -947955085;
				anInt2609 = class536_sub33.readUnsignedShort((short) -4763) * 465590379;
				anInt2613 = class536_sub33.readShort((byte) -32) * -1926815261;
			}
			anInt2723 = class536_sub33.method9720((byte) 1) * -726203335;
			if (0 != aByte2565)
				anInt2614 = class536_sub33.readUnsignedShort((short) -6200) * 170735961;
			if (0 != aByte2618)
				anInt2615 = class536_sub33.readUnsignedShort((short) 17817) * -118712993;
		}
		if (anInt2728 * 1049444347 == 4) {
			anInt2636 = class536_sub33.method9720((byte) 1) * 1355450591;
			if (i_3_ >= 2)
				aBool2637 = class536_sub33.readUnsignedByte(1727619707) == 1;
			aString2638 = class536_sub33.readString((byte) 30);
			anInt2721 = class536_sub33.readUnsignedByte(-1831033049) * -1326793683;
			anInt2640 = class536_sub33.readUnsignedByte(-1598593184) * -70230299;
			anInt2641 = class536_sub33.readUnsignedByte(-1286639781) * -358806889;
			aBool2644 = class536_sub33.readUnsignedByte(-1578005988) == 1;
			anInt2587 = class536_sub33.readInt(-1972013149) * 1647150685;
			anInt2589 = class536_sub33.readUnsignedByte(1689006356) * 1266192547;
			if (i_3_ >= 0)
				anInt2643 = class536_sub33.readUnsignedByte(-1244742356) * -1108499687;
		}
		if (3 == 1049444347 * anInt2728) {
			anInt2587 = class536_sub33.readInt(-1140520189) * 1647150685;
			aBool2642 = class536_sub33.readUnsignedByte(-1009674649) == 1;
			anInt2589 = class536_sub33.readUnsignedByte(-33932256) * 1266192547;
		}
		if (1049444347 * anInt2728 == 9) {
			anInt2590 = class536_sub33.readUnsignedByte(1492992735) * -1272549053;
			anInt2587 = class536_sub33.readInt(-1528516004) * 1647150685;
			aBool2692 = class536_sub33.readUnsignedByte(-241649626) == 1;
		}
		int i_7_ = class536_sub33.read24BitUnsignedInteger(1115932125);
		int i_8_ = class536_sub33.readUnsignedByte(1844911690);
		if (i_8_ != 0) {
			aByteArrayArray2647 = new byte[11][];
			aByteArrayArray2648 = new byte[11][];
			anIntArray2560 = new int[11];
			anIntArray2651 = new int[11];
			for (/**/; i_8_ != 0; i_8_ = class536_sub33.readUnsignedByte(410815387)) {
				int i_9_ = (i_8_ >> 4) - 1;
				i_8_ = i_8_ << 8 | class536_sub33.readUnsignedByte(-28992302);
				i_8_ &= 0xfff;
				if (4095 == i_8_)
					i_8_ = -1;
				byte i_10_ = class536_sub33.readByte(-128398810);
				if (0 != i_10_)
					aBool2582 = true;
				byte i_11_ = class536_sub33.readByte(1686079347);
				anIntArray2560[i_9_] = i_8_;
				aByteArrayArray2647[i_9_] = new byte[] { i_10_ };
				aByteArrayArray2648[i_9_] = new byte[] { i_11_ };
			}
		}
		aString2556 = class536_sub33.readString((byte) 26);
		int i_12_ = class536_sub33.readUnsignedByte(605834642);
		int i_13_ = i_12_ & 0xf;
		int i_14_ = i_12_ >> 4;
		if (i_13_ > 0) {
			aStringArray2657 = new String[i_13_];
			for (int i_15_ = 0; i_15_ < i_13_; i_15_++)
				aStringArray2657[i_15_] = class536_sub33.readString((byte) 116);
		}
		if (i_14_ > 0) {
			int i_16_ = class536_sub33.readUnsignedByte(-273576654);
			anIntArray2668 = new int[1 + i_16_];
			for (int i_17_ = 0; i_17_ < anIntArray2668.length; i_17_++)
				anIntArray2668[i_17_] = -1;
			anIntArray2668[i_16_] = class536_sub33.readUnsignedShort((short) -21634);
		}
		if (i_14_ > 1) {
			int i_18_ = class536_sub33.readUnsignedByte(1427587052);
			anIntArray2668[i_18_] = class536_sub33.readUnsignedShort((short) -8259);
		}
		aString2566 = class536_sub33.readString((byte) 2);
		if (aString2566.equals(""))
			aString2566 = null;
		anInt2600 = class536_sub33.readUnsignedByte(1338867171) * -881661827;
		anInt2663 = class536_sub33.readUnsignedByte(720217478) * 1261514709;
		anInt2534 = class536_sub33.readUnsignedByte(951598298) * 52080145;
		aString2665 = class536_sub33.readString((byte) 79);
		int i_19_ = -1;
		if (Class703.method8258(i_7_, 2109459857) != 0) {
			i_19_ = class536_sub33.readUnsignedShort((short) -13737);
			if (65535 == i_19_)
				i_19_ = -1;
			anInt2626 = class536_sub33.readUnsignedShort((short) 1962) * 323164283;
			if (65535 == anInt2626 * -1130821965)
				anInt2626 = -323164283;
			anInt2580 = class536_sub33.readUnsignedShort((short) -9195) * -979286481;
			if (-652492593 * anInt2580 == 65535)
				anInt2580 = 979286481;
		}
		if (i_3_ >= 0) {
			anInt2713 = class536_sub33.readUnsignedShort((short) -25326) * 1993579121;
			if (1973741201 * anInt2713 == 65535)
				anInt2713 = -1993579121;
		}
		aClass536_Sub14_2575 = new Class536_Sub14(i_7_, i_19_);
		if (i_3_ >= 0) {
			int i_20_ = class536_sub33.readUnsignedByte(-1727562613);
			for (int i_21_ = 0; i_21_ < i_20_; i_21_++) {
				int i_22_ = class536_sub33.read24BitUnsignedInteger(773993057);
				int i_23_ = class536_sub33.readInt(-2060734893);
				aClass4_2630.method560(new Class536_Sub26(i_23_), (long) i_22_);
			}
			int i_24_ = class536_sub33.readUnsignedByte(927854925);
			for (int i_25_ = 0; i_25_ < i_24_; i_25_++) {
				int i_26_ = class536_sub33.read24BitUnsignedInteger(1184258397);
				String string = class536_sub33.method9714((byte) 29);
				aClass4_2630.method560(new Class536_Sub13(string), (long) i_26_);
			}
		}
		anObjectArray2667 = method3467(class536_sub33, (byte) 56);
		anObjectArray2672 = method3467(class536_sub33, (byte) 56);
		anObjectArray2674 = method3467(class536_sub33, (byte) 56);
		anObjectArray2678 = method3467(class536_sub33, (byte) 56);
		anObjectArray2740 = method3467(class536_sub33, (byte) 56);
		anObjectArray2679 = method3467(class536_sub33, (byte) 56);
		anObjectArray2669 = method3467(class536_sub33, (byte) 56);
		anObjectArray2683 = method3467(class536_sub33, (byte) 56);
		anObjectArray2543 = method3467(class536_sub33, (byte) 56);
		anObjectArray2705 = method3467(class536_sub33, (byte) 56);
		if (i_3_ >= 0)
			anObjectArray2624 = method3467(class536_sub33, (byte) 56);
		anObjectArray2673 = method3467(class536_sub33, (byte) 56);
		anObjectArray2555 = method3467(class536_sub33, (byte) 56);
		anObjectArray2645 = method3467(class536_sub33, (byte) 56);
		anObjectArray2670 = method3467(class536_sub33, (byte) 56);
		anObjectArray2696 = method3467(class536_sub33, (byte) 56);
		anObjectArray2675 = method3467(class536_sub33, (byte) 56);
		anObjectArray2676 = method3467(class536_sub33, (byte) 56);
		anObjectArray2715 = method3467(class536_sub33, (byte) 56);
		anObjectArray2662 = method3467(class536_sub33, (byte) 56);
		anObjectArray2710 = method3467(class536_sub33, (byte) 56);
		anIntArray2680 = method3468(class536_sub33, 1944166389);
		anIntArray2561 = method3468(class536_sub33, -547351737);
		anIntArray2684 = method3468(class536_sub33, 1025351193);
		anIntArray2686 = method3468(class536_sub33, 1645875915);
		anIntArray2688 = method3468(class536_sub33, 113886516);
	}

	Object[] method3467(RSByteBuffer class536_sub33, byte i) {
		int i_27_ = class536_sub33.readUnsignedByte(-1076812213);
		if (i_27_ == 0)
			return null;
		Object[] objects = new Object[i_27_];
		for (int i_28_ = 0; i_28_ < i_27_; i_28_++) {
			int i_29_ = class536_sub33.readUnsignedByte(174804425);
			if (0 == i_29_)
				objects[i_28_] = new Integer(class536_sub33.readInt(-1416406806));
			else if (1 == i_29_)
				objects[i_28_] = class536_sub33.readString((byte) 3);
		}
		aBool2666 = true;
		return objects;
	}

	int[] method3468(RSByteBuffer class536_sub33, int i) {
		int i_30_ = class536_sub33.readUnsignedByte(-162052044);
		if (i_30_ == 0)
			return null;
		int[] is = new int[i_30_];
		for (int i_31_ = 0; i_31_ < i_30_; i_31_++)
			is[i_31_] = class536_sub33.readInt(-1690033927);
		return is;
	}

	public Class251() {
		anInt2546 = -724931107;
		anInt2562 = 0;
		aByte2563 = (byte) 0;
		aByte2564 = (byte) 0;
		aByte2565 = (byte) 0;
		aByte2618 = (byte) 0;
		anInt2567 = 0;
		anInt2655 = 0;
		anInt2569 = 0;
		anInt2664 = 0;
		anInt2571 = 0;
		anInt2572 = 0;
		anInt2573 = 0;
		anInt2574 = 0;
		anInt2693 = -1035323945;
		anInt2576 = -1090634513;
		anInt2577 = -1248497819;
		aBool2578 = false;
		anInt2626 = -323164283;
		anInt2580 = 979286481;
		anInt2713 = -1993579121;
		aBool2708 = false;
		anInt2581 = 0;
		anInt2584 = 0;
		anInt2585 = 0;
		anInt2586 = 0;
		anInt2587 = 0;
		aBool2642 = false;
		anInt2589 = 0;
		anInt2590 = -1272549053;
		aBool2692 = false;
		anInt2592 = 2093350021;
		anInt2593 = 0;
		aBool2594 = false;
		anInt2720 = 0;
		anInt2596 = 0;
		aBool2649 = false;
		aBool2677 = true;
		anInt2601 = -1782707337;
		anInt2603 = -721316367;
		anInt2658 = 0;
		anInt2606 = 0;
		anInt2551 = 0;
		anInt2547 = 0;
		anInt2609 = 0;
		anInt2610 = 0;
		anInt2611 = 0;
		anInt2612 = 0;
		anInt2613 = 592002220;
		anInt2614 = 0;
		anInt2615 = 0;
		aBool2616 = false;
		aBool2682 = false;
		anInt2595 = -1360798542;
		aBool2659 = false;
		anInt2636 = -1355450591;
		aBool2637 = true;
		aString2638 = "";
		anInt2721 = 0;
		anInt2640 = 0;
		anInt2641 = 0;
		aBool2644 = false;
		anInt2643 = 0;
		aBool2608 = false;
		aClass536_Sub14_2575 = Class536_Sub14.aClass536_Sub14_10477;
		aBool2582 = false;
		aString2556 = "";
		anInt2660 = -726833675;
		aClass251_2553 = null;
		anInt2600 = 0;
		anInt2663 = 0;
		anInt2534 = anInt2617 * -1219474557;
		aString2665 = "";
		aBool2666 = false;
		anInt2711 = 180322533;
		anInt2712 = 0;
		aString2699 = null;
		aBool2671 = false;
		anInt2716 = -1848781035;
		anInt2604 = 1513859745;
		anInt2723 = 726203335;
		aBool2727 = false;
		aBool2607 = false;
		anInt2729 = 1185389879;
		anInt2730 = 0;
		anInt2731 = 0;
		anInt2732 = 0;
		anInt2733 = 0;
		anInt2734 = 0;
		anInt2735 = 0;
		anInt2736 = 1381511251;
		anInt2627 = 1535001011;
	}

	public Class184 method3469(Class410 class410, Interface51 interface51, byte i) {
		Class184 class184 = ((Class184) class410.method4980(interface51, 2003514143 * anInt2636, false, aBool2637, (byte) -19));
		aBool2724 = null == class184;
		return class184;
	}

	public Class2 method3470(Class410 class410, Interface51 interface51, byte i) {
		Class2 class2 = class410.method4981(interface51, anInt2636 * 2003514143, -1014405925);
		aBool2724 = class2 == null;
		return class2;
	}

	public void method3471(Class167 class167, Class177 class177, Class433 class433, int i, int i_32_) {
		class177.method2513(class433);
		Class172[] class172s = class177.method2514();
		Class156[] class156s = class177.method2515();
		if ((aClass627_2739 == null || aClass627_2739.aBool8168) && (null != class172s || class156s != null))
			aClass627_2739 = Class627.method7484(i, false);
		if (null != aClass627_2739)
			aClass627_2739.method7474(class167, (long) i, class172s, class156s, false);
	}

	public Class507 method3472(Class34_Sub9 class34_sub9, Class34_Sub12 class34_sub12, byte i) {
		if (-1 == 2028683203 * anInt2716)
			return null;
		long l = (((long) (-920532679 * anInt2719) & 0xffffL) << 48 | ((long) (anInt2737 * 1957590239) & 0xffffL) << 32 | ((long) (492400851 * anInt2700) & 0xffffL) << 16 | (long) (2028683203 * anInt2716) & 0xffffL);
		Class507 class507 = (Class507) aClass199_2554.method2886(l);
		if (null == class507) {
			class507 = Class205_Sub15.method9085(2028683203 * anInt2716, anInt2719 * -920532679, anInt2737 * 1957590239, 492400851 * anInt2700, class34_sub9, class34_sub12, 2086556901);
			aClass199_2554.method2881(class507, l);
		}
		return class507;
	}

	public void method3473(int i) {
		anObjectArray2667 = null;
		anObjectArray2555 = null;
		anObjectArray2645 = null;
		anObjectArray2670 = null;
		anObjectArray2696 = null;
		anObjectArray2672 = null;
		anObjectArray2673 = null;
		anObjectArray2674 = null;
		anObjectArray2675 = null;
		anObjectArray2676 = null;
		anObjectArray2740 = null;
		anObjectArray2678 = null;
		anObjectArray2679 = null;
		anIntArray2680 = null;
		anInt2730 = 0;
		anObjectArray2669 = null;
		anIntArray2561 = null;
		anInt2731 = 0;
		anObjectArray2683 = null;
		anIntArray2684 = null;
		anInt2732 = 0;
		anObjectArray2662 = null;
		anIntArray2686 = null;
		anInt2733 = 0;
		anObjectArray2710 = null;
		anIntArray2688 = null;
		anInt2734 = 0;
		anObjectArray2689 = null;
		anIntArray2690 = null;
		anInt2735 = 0;
		anObjectArray2543 = null;
		anObjectArray2705 = null;
		anObjectArray2624 = null;
		anObjectArray2715 = null;
		anObjectArray2695 = null;
		anObjectArray2538 = null;
		anObjectArray2697 = null;
		anObjectArray2698 = null;
		anObjectArray2544 = null;
		anObjectArray2701 = null;
		anObjectArray2628 = null;
		anObjectArray2702 = null;
		anObjectArray2703 = null;
		anObjectArray2704 = null;
		anObjectArray2661 = null;
		anObjectArray2706 = null;
		anObjectArray2707 = null;
		anObjectArray2599 = null;
		anObjectArray2709 = null;
		anInt2729 = 1185389879;
	}

	public void method3474(int i, String string, int i_33_) {
		if (null == aStringArray2657 || aStringArray2657.length <= i) {
			String[] strings = new String[i + 1];
			if (aStringArray2657 != null) {
				for (int i_34_ = 0; i_34_ < aStringArray2657.length; i_34_++)
					strings[i_34_] = aStringArray2657[i_34_];
			}
			aStringArray2657 = strings;
		}
		aStringArray2657[i] = string;
	}

	public void method3475(int i, int i_35_, int i_36_) {
		if (null == anIntArray2668 || anIntArray2668.length <= i) {
			int[] is = new int[i + 1];
			if (null != anIntArray2668) {
				for (int i_37_ = 0; i_37_ < anIntArray2668.length; i_37_++)
					is[i_37_] = anIntArray2668[i_37_];
				for (int i_38_ = anIntArray2668.length; i_38_ < i; i_38_++)
					is[i_38_] = -1;
			}
			anIntArray2668 = is;
		}
		anIntArray2668[i] = i_35_;
	}

	public int method3476(int i, int i_39_, int i_40_) {
		if (aClass4_2630 == null)
			return i_39_;
		Class536_Sub26 class536_sub26 = (Class536_Sub26) aClass4_2630.method556((long) i);
		if (null == class536_sub26)
			return i_39_;
		return 2083602213 * class536_sub26.anInt10533;
	}

	public String method3477(int i, String string, int i_41_) {
		if (aClass4_2630 == null)
			return string;
		Class536_Sub13 class536_sub13 = (Class536_Sub13) aClass4_2630.method556((long) i);
		if (class536_sub13 == null)
			return string;
		return (String) class536_sub13.anObject10468;
	}

	public Class244 method3478(Class167 class167, int i) {
		long l = ((long) (-1591767037 * anInt2559) << 32 | (long) (-809980533 * anInt2546) & 0xffffffffL);
		Class244 class244 = (Class244) aClass199_2568.method2886(l);
		if (class244 != null) {
			if (-1307221069 * anInt2592 != class244.anInt2459 * -959136425) {
				aClass199_2568.method2879(l);
				class244 = null;
			}
			if (class244 != null)
				return class244;
		}
		Class179 class179 = Class181.method2718(Class252.aClass461_2745, -1307221069 * anInt2592, 0);
		if (null == class179)
			return null;
		int i_42_ = class179.method2640();
		int i_43_ = class179.method2671();
		int[] is = new int[i_43_];
		int[] is_44_ = new int[i_43_];
		for (int i_45_ = 0; i_45_ < class179.method2677(); i_45_++) {
			int i_46_ = 0;
			int i_47_ = class179.method2633();
			for (int i_48_ = 0; i_48_ < class179.method2633(); i_48_++) {
				if (class179.method2650(i_48_, i_45_) != 0) {
					i_46_ = i_48_;
					break;
				}
			}
			for (int i_49_ = class179.method2633() - 1; i_49_ >= i_46_; i_49_--) {
				if (class179.method2650(i_49_, i_45_) != 0) {
					i_47_ = i_49_ + 1;
					break;
				}
			}
			is[i_45_ + class179.method2637()] = i_46_ + class179.method2635();
			is_44_[i_45_ + class179.method2637()] = i_47_ - i_46_;
		}
		Class161 class161 = class167.method2087(i_42_, i_43_, is, is_44_);
		if (class161 == null)
			return null;
		class244 = new Class244(i_42_, i_43_, is_44_, is, class161, -1307221069 * anInt2592);
		aClass199_2568.method2881(class244, l);
		return class244;
	}

	public void method3479(int i, String string, int i_50_) {
		if (null == aClass4_2630) {
			aClass4_2630 = new Class4(16);
			aClass4_2630.method560(new Class536_Sub13(string), (long) i);
		} else {
			Class536_Sub13 class536_sub13 = (Class536_Sub13) aClass4_2630.method556((long) i);
			if (null != class536_sub13)
				class536_sub13.method6484(1521060553);
			aClass4_2630.method560(new Class536_Sub13(string), (long) i);
		}
	}

	public static void method3480() {
		Class463.aClass234Array5227 = new Class234[Class452.aClass461_4953.method5570(7368056)];
		Class684.aBoolArray8653 = new boolean[Class452.aClass461_4953.method5570(-2108077950)];
	}

	public void method3481(int i, short i_51_, short i_52_, byte i_53_) {
		if (i < 5) {
			if (aShortArray2621 == null) {
				aShortArray2621 = new short[5];
				aShortArray2622 = new short[5];
			}
			aShortArray2621[i] = i_51_;
			aShortArray2622[i] = i_52_;
		}
	}

	static {
		anInt2533 = -1602743979;
		anInt2535 = 693108138;
		anInt2634 = -210346425;
		anInt2536 = -224505192;
		anInt2537 = -1045695779;
		anInt2646 = 1278182445;
		anInt2691 = -121753538;
		anInt2540 = 1053910947;
		anInt2541 = -1446660264;
		anInt2542 = -1459567077;
		anInt2685 = 636577971;
		anInt2605 = 498335138;
		anInt2545 = 1964252795;
		anInt2570 = 644021888;
		anInt2539 = -1546448705;
		anInt2548 = -337379610;
		anInt2549 = -1251415409;
		aClass199_2552 = new Class199(6000000, 200);
		aClass199_2568 = new Class199(8);
		aClass199_2554 = new Class199(4);
		aClass199_2694 = new Class199(50);
		aBool2724 = false;
	}

	public static void method3482() {
		aClass199_2552.method2884((byte) -84);
		aClass199_2694.method2884((byte) -93);
		aClass199_2568.method2884((byte) -68);
		aClass199_2554.method2884((byte) -51);
	}

	static void method3483(int i) {
		if (-1 != i && !Class684.aBoolArray8653[i]) {
			Class452.aClass461_4953.method5568(i, (short) -28273);
			Class463.aClass234Array5227[i] = null;
		}
	}

	public Class143 method3484(Class167 class167, int i) {
		aBool2724 = false;
		long l = (((long) (1514577167 * anInt2596) << 40) + (((aBool2597 ? 1L : 0L) << 38) + (((long) (1960646061 * anInt2720) << 36) + ((long) (anInt2592 * -1307221069) + ((aBool2649 ? 1L : 0L) << 35))) + ((aBool2598 ? 1L : 0L) << 39)));
		Class143 class143 = (Class143) aClass199_2552.method2886(l);
		if (class143 != null)
			return class143;
		Class179 class179 = Class181.method2718(Class252.aClass461_2745, -1307221069 * anInt2592, 0);
		if (class179 == null) {
			aBool2724 = true;
			return null;
		}
		if (aBool2597)
			class179.method2646();
		if (aBool2598)
			class179.method2648();
		if (anInt2720 * 1960646061 > 0)
			class179.method2642(anInt2720 * 1960646061);
		else if (1514577167 * anInt2596 != 0)
			class179.method2642(1);
		if (anInt2720 * 1960646061 >= 1)
			class179.method2655(1);
		if (anInt2720 * 1960646061 >= 2)
			class179.method2655(16777215);
		if (0 != anInt2596 * 1514577167)
			class179.method2644(~0xffffff | anInt2596 * 1514577167);
		class143 = class167.method2082(class179, true);
		aClass199_2552.method2882(class143, l, (class143.method1720() * class143.method1752() * 4), (byte) 122);
		return class143;
	}

	static void method3485(int i) {
		if (-1 != i && !Class684.aBoolArray8653[i]) {
			Class452.aClass461_4953.method5568(i, (short) -7355);
			Class463.aClass234Array5227[i] = null;
		}
	}

	static void method3486(int i) {
		if (-1 != i && !Class684.aBoolArray8653[i]) {
			Class452.aClass461_4953.method5568(i, (short) -28149);
			Class463.aClass234Array5227[i] = null;
		}
	}

	public void method3487(int i, int i_54_, int i_55_) {
		if (aClass4_2630 == null) {
			aClass4_2630 = new Class4(16);
			aClass4_2630.method560(new Class536_Sub26(i_54_), (long) i);
		} else {
			Class536_Sub26 class536_sub26 = (Class536_Sub26) aClass4_2630.method556((long) i);
			if (class536_sub26 == null)
				aClass4_2630.method560(new Class536_Sub26(i_54_), (long) i);
			else
				class536_sub26.anInt10533 = 1189135533 * i_54_;
		}
	}

	public static void method3488(int i) {
		Class684.aBoolArray8653[i] = false;
		Class265.method3688(i, (byte) 99);
	}

	static void method3489(int i) {
		if (-1 != i && !Class684.aBoolArray8653[i]) {
			Class452.aClass461_4953.method5568(i, (short) -11962);
			Class463.aClass234Array5227[i] = null;
		}
	}

	public static void method3490() {
		aClass199_2552.method2884((byte) -14);
		aClass199_2694.method2884((byte) -83);
		aClass199_2568.method2884((byte) -73);
		aClass199_2554.method2884((byte) -95);
	}

	public static void method3491(int i) {
		Class649_Sub1_Sub2_Sub1.anInt11973 = -738987877 * i;
		aClass199_2694.method2884((byte) -43);
	}

	public static void method3492(int i) {
		aClass199_2552.method2883(i, -1845389226);
		aClass199_2694.method2883(i, -2132298936);
		aClass199_2568.method2883(i, -1892864067);
		aClass199_2554.method2883(i, -2021350877);
	}

	public static void method3493(int i) {
		Class649_Sub1_Sub2_Sub1.anInt11973 = -738987877 * i;
		aClass199_2694.method2884((byte) -82);
	}

	public static void method3494() {
		aClass199_2552.method2884((byte) -28);
		aClass199_2694.method2884((byte) -74);
		aClass199_2568.method2884((byte) -123);
		aClass199_2554.method2884((byte) -18);
	}

	public static void method3495(int i) {
		Class649_Sub1_Sub2_Sub1.anInt11973 = -738987877 * i;
		aClass199_2694.method2884((byte) -5);
	}

	public static void method3496() {
		aClass199_2552.method2892(2118310749);
		aClass199_2694.method2892(1711467257);
		aClass199_2568.method2892(-10238739);
		aClass199_2554.method2892(1115818597);
	}

	public void method3497(int i, byte i_56_) {
		if (aClass4_2630 != null) {
			Class536 class536 = aClass4_2630.method556((long) i);
			if (null != class536)
				class536.method6484(2035415367);
		}
	}

	public void method3498() {
		anObjectArray2667 = null;
		anObjectArray2555 = null;
		anObjectArray2645 = null;
		anObjectArray2670 = null;
		anObjectArray2696 = null;
		anObjectArray2672 = null;
		anObjectArray2673 = null;
		anObjectArray2674 = null;
		anObjectArray2675 = null;
		anObjectArray2676 = null;
		anObjectArray2740 = null;
		anObjectArray2678 = null;
		anObjectArray2679 = null;
		anIntArray2680 = null;
		anInt2730 = 0;
		anObjectArray2669 = null;
		anIntArray2561 = null;
		anInt2731 = 0;
		anObjectArray2683 = null;
		anIntArray2684 = null;
		anInt2732 = 0;
		anObjectArray2662 = null;
		anIntArray2686 = null;
		anInt2733 = 0;
		anObjectArray2710 = null;
		anIntArray2688 = null;
		anInt2734 = 0;
		anObjectArray2689 = null;
		anIntArray2690 = null;
		anInt2735 = 0;
		anObjectArray2543 = null;
		anObjectArray2705 = null;
		anObjectArray2624 = null;
		anObjectArray2715 = null;
		anObjectArray2695 = null;
		anObjectArray2538 = null;
		anObjectArray2697 = null;
		anObjectArray2698 = null;
		anObjectArray2544 = null;
		anObjectArray2701 = null;
		anObjectArray2628 = null;
		anObjectArray2702 = null;
		anObjectArray2703 = null;
		anObjectArray2704 = null;
		anObjectArray2661 = null;
		anObjectArray2706 = null;
		anObjectArray2707 = null;
		anObjectArray2599 = null;
		anObjectArray2709 = null;
		anInt2729 = 1185389879;
	}

	public void method3499(int i, int i_57_) {
		if (null == anIntArray2668 || anIntArray2668.length <= i) {
			int[] is = new int[i + 1];
			if (null != anIntArray2668) {
				for (int i_58_ = 0; i_58_ < anIntArray2668.length; i_58_++)
					is[i_58_] = anIntArray2668[i_58_];
				for (int i_59_ = anIntArray2668.length; i_59_ < i; i_59_++)
					is[i_59_] = -1;
			}
			anIntArray2668 = is;
		}
		anIntArray2668[i] = i_57_;
	}

	public void method3500(int i, int i_60_) {
		if (null == anIntArray2668 || anIntArray2668.length <= i) {
			int[] is = new int[i + 1];
			if (null != anIntArray2668) {
				for (int i_61_ = 0; i_61_ < anIntArray2668.length; i_61_++)
					is[i_61_] = anIntArray2668[i_61_];
				for (int i_62_ = anIntArray2668.length; i_62_ < i; i_62_++)
					is[i_62_] = -1;
			}
			anIntArray2668 = is;
		}
		anIntArray2668[i] = i_60_;
	}

	public static void method3501(int i) {
		Class684.aBoolArray8653[i] = false;
		Class265.method3688(i, (byte) 81);
	}

	public Class177 method3502(Class167 class167, int i, Class34_Sub16 class34_sub16, Class34_Sub2 class34_sub2, Class34_Sub7 class34_sub7, Class34_Sub13 class34_sub13, Class34_Sub11 class34_sub11, Interface20 interface20, Interface18 interface18, Class688 class688, Class617 class617, short i_63_) {
		aBool2724 = false;
		if (0 == 1939619399 * anInt2601)
			return null;
		if (1 == anInt2601 * 1939619399 && anInt2602 * -2011124813 == -1)
			return null;
		if (1939619399 * anInt2601 == 1) {
			int i_64_ = i;
			if (null != class688)
				i |= class688.method8162((byte) 34);
			long l = -1L;
			long[] ls = RSByteBuffer.aLongArray10595;
			if (null != aShortArray2619) {
				for (int i_65_ = 0; i_65_ < aShortArray2619.length; i_65_++) {
					l = l >>> 8 ^ ls[(int) ((l ^ (long) (aShortArray2619[i_65_] >> 8)) & 0xffL)];
					l = l >>> 8 ^ ls[(int) ((l ^ (long) aShortArray2619[i_65_]) & 0xffL)];
					l = l >>> 8 ^ ls[(int) ((l ^ (long) (aShortArray2620[i_65_] >> 8)) & 0xffL)];
					l = l >>> 8 ^ ls[(int) ((l ^ (long) aShortArray2620[i_65_]) & 0xffL)];
				}
				i |= 0x4000;
			}
			if (null != aShortArray2621) {
				for (int i_66_ = 0; i_66_ < aShortArray2621.length; i_66_++) {
					l = l >>> 8 ^ ls[(int) ((l ^ (long) (aShortArray2621[i_66_] >> 8)) & 0xffL)];
					l = l >>> 8 ^ ls[(int) ((l ^ (long) aShortArray2621[i_66_]) & 0xffL)];
					l = l >>> 8 ^ ls[(int) ((l ^ (long) (aShortArray2622[i_66_] >> 8)) & 0xffL)];
					l = l >>> 8 ^ ls[(int) ((l ^ (long) aShortArray2622[i_66_]) & 0xffL)];
				}
				i |= 0x8000;
			}
			long l_67_ = ((long) (class167.anInt1850 * 1526643673) << 59 | (long) (1939619399 * anInt2601) << 55 | (long) (anInt2602 * -2011124813) << 38 | l & 0x3fffffffffL);
			Class177 class177 = (Class177) aClass199_2694.method2886(l_67_);
			if (null == class177 || class167.method2308(class177.method2466(), i) != 0) {
				if (class177 != null)
					i = class167.method2068(i, class177.method2466());
				Class180 class180 = Class180.method2708(Class537.aClass461_7152, -2011124813 * anInt2602, 0);
				if (class180 == null) {
					aBool2724 = true;
					return null;
				}
				if (class180.anInt1949 < 13)
					class180.method2707(2);
				class177 = class167.method2093(class180, i, (Class649_Sub1_Sub2_Sub1.anInt11973 * -1304267373), 64, 768);
				if (null != aShortArray2619) {
					for (int i_68_ = 0; i_68_ < aShortArray2619.length; i_68_++)
						class177.method2507(aShortArray2619[i_68_], aShortArray2620[i_68_]);
				}
				if (aShortArray2621 != null) {
					for (int i_69_ = 0; i_69_ < aShortArray2621.length; i_69_++)
						class177.method2509(aShortArray2621[i_69_], aShortArray2622[i_69_]);
				}
				aClass199_2694.method2881(class177, l_67_);
			}
			if (null != class688) {
				class177 = class177.method2623((byte) 1, i, true);
				class688.method8132(class177, 0, -2055911089);
			}
			class177.method2465(i_64_);
			return class177;
		}
		if (2 == anInt2601 * 1939619399) {
			Class177 class177 = (((Class296) class34_sub7.method70(anInt2602 * -2011124813, (byte) -2)).method3981(class167, i, interface20, interface18, class688, aClass304_2583, (short) -15253));
			if (class177 == null) {
				aBool2724 = true;
				return null;
			}
			return class177;
		}
		if (3 == anInt2601 * 1939619399) {
			if (null == class617)
				return null;
			Class177 class177 = class617.method7346(class167, i, class34_sub2, class34_sub7, class34_sub13, interface20, interface18, class688, -211912518);
			if (class177 == null) {
				aBool2724 = true;
				return null;
			}
			return class177;
		}
		if (4 == anInt2601 * 1939619399) {
			Class1 class1 = ((Class1) class34_sub13.method70(-2011124813 * anInt2602, (byte) 6));
			Class177 class177 = class1.method503(class167, i, 10, class617, class688, 0, 0, 0, 0, (byte) 35);
			if (null == class177) {
				aBool2724 = true;
				return null;
			}
			return class177;
		}
		if (1939619399 * anInt2601 == 6) {
			Class177 class177 = (((Class296) class34_sub7.method70(-2011124813 * anInt2602, (byte) 24)).method3995(class167, i, class34_sub16, interface20, interface18, class688, null, null, null, 0, aClass304_2583, 978684503));
			if (null == class177) {
				aBool2724 = true;
				return null;
			}
			return class177;
		}
		if (anInt2601 * 1939619399 == 7) {
			if (null == class617)
				return null;
			int i_70_ = anInt2602 * -2011124813 >>> 16;
			int i_71_ = anInt2602 * -2011124813 & 0xffff;
			int i_72_ = 2109818095 * anInt2603;
			Class177 class177 = class617.method7347(class167, i, class34_sub2, class34_sub11, class688, i_70_, i_71_, i_72_, -1539494094);
			if (null == class177) {
				aBool2724 = true;
				return null;
			}
			return class177;
		}
		return null;
	}

	public void method3503(int i, int i_73_) {
		if (aClass4_2630 == null) {
			aClass4_2630 = new Class4(16);
			aClass4_2630.method560(new Class536_Sub26(i_73_), (long) i);
		} else {
			Class536_Sub26 class536_sub26 = (Class536_Sub26) aClass4_2630.method556((long) i);
			if (class536_sub26 == null)
				aClass4_2630.method560(new Class536_Sub26(i_73_), (long) i);
			else
				class536_sub26.anInt10533 = 1189135533 * i_73_;
		}
	}

	public void method3504(int i, int i_74_) {
		if (aClass4_2630 == null) {
			aClass4_2630 = new Class4(16);
			aClass4_2630.method560(new Class536_Sub26(i_74_), (long) i);
		} else {
			Class536_Sub26 class536_sub26 = (Class536_Sub26) aClass4_2630.method556((long) i);
			if (class536_sub26 == null)
				aClass4_2630.method560(new Class536_Sub26(i_74_), (long) i);
			else
				class536_sub26.anInt10533 = 1189135533 * i_74_;
		}
	}

	public void method3505() {
		anObjectArray2667 = null;
		anObjectArray2555 = null;
		anObjectArray2645 = null;
		anObjectArray2670 = null;
		anObjectArray2696 = null;
		anObjectArray2672 = null;
		anObjectArray2673 = null;
		anObjectArray2674 = null;
		anObjectArray2675 = null;
		anObjectArray2676 = null;
		anObjectArray2740 = null;
		anObjectArray2678 = null;
		anObjectArray2679 = null;
		anIntArray2680 = null;
		anInt2730 = 0;
		anObjectArray2669 = null;
		anIntArray2561 = null;
		anInt2731 = 0;
		anObjectArray2683 = null;
		anIntArray2684 = null;
		anInt2732 = 0;
		anObjectArray2662 = null;
		anIntArray2686 = null;
		anInt2733 = 0;
		anObjectArray2710 = null;
		anIntArray2688 = null;
		anInt2734 = 0;
		anObjectArray2689 = null;
		anIntArray2690 = null;
		anInt2735 = 0;
		anObjectArray2543 = null;
		anObjectArray2705 = null;
		anObjectArray2624 = null;
		anObjectArray2715 = null;
		anObjectArray2695 = null;
		anObjectArray2538 = null;
		anObjectArray2697 = null;
		anObjectArray2698 = null;
		anObjectArray2544 = null;
		anObjectArray2701 = null;
		anObjectArray2628 = null;
		anObjectArray2702 = null;
		anObjectArray2703 = null;
		anObjectArray2704 = null;
		anObjectArray2661 = null;
		anObjectArray2706 = null;
		anObjectArray2707 = null;
		anObjectArray2599 = null;
		anObjectArray2709 = null;
		anInt2729 = 1185389879;
	}

	public static void method3506() {
		Class463.aClass234Array5227 = new Class234[Class452.aClass461_4953.method5570(-1681282764)];
		Class684.aBoolArray8653 = new boolean[Class452.aClass461_4953.method5570(1872643045)];
	}

	public void method3507(int i) {
		if (aClass4_2630 != null) {
			Class536 class536 = aClass4_2630.method556((long) i);
			if (null != class536)
				class536.method6484(-1207226900);
		}
	}

	public void method3508(int i) {
		if (aClass4_2630 != null) {
			Class536 class536 = aClass4_2630.method556((long) i);
			if (null != class536)
				class536.method6484(2107396312);
		}
	}

	public void method3509(int i) {
		if (aClass4_2630 != null) {
			Class536 class536 = aClass4_2630.method556((long) i);
			if (null != class536)
				class536.method6484(140287130);
		}
	}

	static void method3510(Class668 class668, byte i) {
		class668.anIntArray8541[1867269829 * class668.anInt8542 - 1] = (((Class277) Class38.aClass53_Sub2_481.method70((class668.anIntArray8541[((1867269829 * class668.anInt8542) - 1)]), (byte) 104)).method3745(Class465_Sub1.aClass96_10352, 1025086024) ? 1 : 0);
	}

	static final void method3511(Class668 class668, int i) {
		class668.anInt8542 -= -1411037171;
	}

	static final void method3512(Class668 class668, byte i) {
		int i_75_ = (class668.anIntArray8541[(class668.anInt8542 -= -1411037171) * 1867269829]);
		int[] is = Class585.method7048(i_75_, 606043886);
		Class687.method8114(is, 0, class668.anIntArray8541, class668.anInt8542 * 1867269829, 3);
		class668.anInt8542 += 61855783;
	}

	static boolean method3513(CharSequence charsequence, int i, boolean bool, int i_76_) {
		if (i < 2 || i > 36)
			throw new IllegalArgumentException(new StringBuilder().append("").append(i).toString());
		boolean bool_77_ = false;
		boolean bool_78_ = false;
		int i_79_ = 0;
		int i_80_ = charsequence.length();
		for (int i_81_ = 0; i_81_ < i_80_; i_81_++) {
			int i_82_ = charsequence.charAt(i_81_);
			if (0 == i_81_) {
				if (45 == i_82_) {
					bool_77_ = true;
					continue;
				}
				if (43 == i_82_ && bool)
					continue;
			}
			if (i_82_ >= 48 && i_82_ <= 57)
				i_82_ -= 48;
			else if (i_82_ >= 65 && i_82_ <= 90)
				i_82_ -= 55;
			else if (i_82_ >= 97 && i_82_ <= 122)
				i_82_ -= 87;
			else
				return false;
			if (i_82_ >= i)
				return false;
			if (bool_77_)
				i_82_ = -i_82_;
			int i_83_ = i_82_ + i_79_ * i;
			if (i_83_ / i != i_79_)
				return false;
			i_79_ = i_83_;
			bool_78_ = true;
		}
		return bool_78_;
	}

	public static void method3514(Class153 class153, byte i) {
		Class536_Sub18_Sub5 class536_sub18_sub5 = Class34_Sub11.method10334(1, (long) (-1786401803 * class153.anInt1715));
		class536_sub18_sub5.method10669(-1056424354);
	}
}
